...
test.describe("Formulário com Validação de CEP", () => {
  ...
  test("deve buscar e preencher endereço ao digitar CEP válido", async ({
    page,
  }) => {
    // Arrange
  ...
  test("deve usar cache na segunda consulta (mais rápido)", async ({
    page,
  }) => {
    // Arrange
    const cepInput = page.locator('input[name="cep"]');
...
test.describe("Formulário com Validação de CNPJ", () => {
  ...
  test("deve validar e preencher dados da empresa com CNPJ válido", async ({
    page,
  }) => {
    // Arrange
...
test.describe("Performance e Acessibilidade", () => {
  test("deve ter boa performance no preenchimento de formulários", async ({
    page,
  }) => {
    await page.goto("http://localhost:4173/");
  ...
  test("deve ser acessível via teclado", async ({ page }) => {
    ...
    const logradouro = await page
      .locator('input[name="logradouro"]')
      .inputValue();
