...
test.describe("Dashboard - Módulos Específicos", () => {
  test("Dashboard Financeiro - deve exibir KPIs financeiros", async ({
    page,
  }) => {
    await page.goto("/dashboard-financeiro");

    await expect(
      page.locator('h1:has-text("Dashboard Financeiro")'),
    ).toBeVisible();
  ...
  test("Dashboard Cirurgias - deve exibir métricas cirúrgicas", async ({
    page,
  }) => {
