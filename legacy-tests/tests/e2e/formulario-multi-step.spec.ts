...
test.describe("Formulário Multi-Step - Cadastro de Paciente", () => {
  ...
  test("deve renderizar o formulário com 4 steps", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Cadastro de Paciente" }),
    ).toBeVisible();
  ...
  test("deve preencher endereço automaticamente ao digitar CEP válido", async ({
    page,
  }) => {
    // Avançar para step 2
    await page.fill("#nome", "João da Silva");
  ...
  test("deve voltar para step anterior ao clicar em Voltar", async ({
    page,
  }) => {
    // Avançar para step 2
    await page.fill("#nome", "João da Silva");
  ...
  test("deve permitir pular para step clicado (se permitido)", async ({
    page,
  }) => {
    // Preencher step 1
    await page.fill("#nome", "João da Silva");
  ...
  test("deve preencher todos os steps e concluir cadastro", async ({
    page,
  }) => {
    ...
    await page.fill(
      "#observacoes",
      "Paciente preferencial, atendimento prioritário.",
    );
  ...
  test("deve exibir progress bar com porcentagem correta", async ({ page }) => {
    ...
    const progressBar = page
      .locator(".neuro-inset .bg-\\[var\\(--primary\\)\\]")
      .first();
  ...
  test("deve renderizar todos os campos do step 3 (Info Médicas)", async ({
    page,
  }) => {
    // Avançar até step 3
    await page.fill("#nome", "João");
  ...
  test("deve ter classes neumórficas em todos os componentes", async ({
    page,
  }) => {
  ...
  test("deve validar campos obrigatórios antes de avançar", async ({
    page,
  }) => {
