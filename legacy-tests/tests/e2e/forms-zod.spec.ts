...
test.describe("Formulários Zod - FormularioMedicoAvancado", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const session = JSON.stringify({
      ...
      permissoes: [
        { codigo: "SYSTEM_ALL", nome: "All", recurso: "*", acao: "all" },
      ],
    ...
    await expect(page.locator('h1:text("Cadastro de Médicos")')).toBeVisible({
      timeout: 15000,
    });
  });
  ...
  test("deve validar email formato", async ({ page }) => {
    ...
    expect(
      isInvalid === null || isInvalid === "true" || isInvalid === "false",
    ).toBe(true);
  });
...
test.describe("Formulários Zod - Validações Customizadas", () => {
  test("deve validar CRM único", async ({ page }) => {
    ...
    await page.addInitScript(
      (value) => window.localStorage.setItem("icarus_session", value as string),
      session,
    );
  ...
  test("deve validar telefone formato brasileiro", async ({ page }) => {
    ...
    if (await telefone.isVisible()) {
      ...
      await page.waitForTimeout(300);
      const hasError =
        (await page.locator("text=/Telefone|Phone|Celular/i").count()) > 0;
  ...
  test("deve exibir loading durante submissão", async ({ page }) => {
    ...
    if (!(await submitBtn.isDisabled())) {
      ...
      const hasLoading = await page
        .locator("text=/Salvando|Loading|Aguarde/i")
