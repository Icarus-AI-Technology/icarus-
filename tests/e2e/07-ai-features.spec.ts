...
test.describe('IA - ChatBot Tutor OPME', () => {
  test('deve exibir interface do chatbot', async ({ page }) => {
    ...
    await expect(page.locator('textarea[placeholder*="pergunta"]')).toBeVisible();
  });
  ...
  test('deve enviar mensagem ao chatbot', async ({ page }) => {
    ...
    // Aguarda resposta
    await expect(page.locator('text=/OPME significa/i')).toBeVisible({ timeout: 5000 });
  });
  ...
  test('deve exibir previsões de estoque', async ({ page }) => {
    ...
    await page.route('**/api/ml/predict-stock**', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          predictions: [
            { item: 'Parafuso Pedicular', demand_forecast: 45, confidence: 0.87 },
  ...
  test('deve sugerir fornecedores', async ({ page }) => {
    ...
    await page.route('**/api/ml/recommend-suppliers**', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          suppliers: [
            { id: '1', name: 'Fornecedor A', score: 0.95, reason: 'Melhor preço' },
            { id: '2', name: 'Fornecedor B', score: 0.88, reason: 'Melhor prazo' },
          ],
        }),
      });
    });
...
test.describe('IA - Automações', () => {
  ...
  test('deve criar nova automação', async ({ page }) => {
    ...
    await expect(page.locator('text=/automação criada/i')).toBeVisible({ timeout: 3000 });
  });
  ...
  test('deve ativar/desativar automação', async ({ page }) => {
    ...
    await page.route('**/api/automations**', async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify([
          { id: '1', name: 'Auto-reorder', active: true },
        ]),
      });
    });
    ...
    const isChecked = await page.locator('[data-automation-id="1"] input[type="checkbox"]').isChecked();
