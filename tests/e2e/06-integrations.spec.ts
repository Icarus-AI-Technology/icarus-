...
test.describe('Integração - OCR (Tesseract)', () => {
  test('deve fazer upload de documento', async ({ page }) => {
    ...
    await expect(page.locator('text=/processando/i')).toBeVisible({ timeout: 3000 });
  });
  ...
  test('deve exibir resultado do OCR', async ({ page }) => {
    ...
    // Aguarda resultado
    await expect(page.locator('text=NOTA FISCAL')).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('Integração - Email (SendGrid)', () => {
  test('deve enviar email de notificação', async ({ page }) => {
    ...
    await expect(page.locator('text=/email enviado/i')).toBeVisible({ timeout: 3000 });
  });
});
...
test.describe('Integração - SMS (Twilio)', () => {
  test('deve enviar SMS', async ({ page }) => {
    ...
    await expect(page.locator('text=/SMS enviado/i')).toBeVisible({ timeout: 3000 });
  });
});
...
test.describe('Integração - GPT Researcher', () => {
  test('deve fazer pesquisa com IA', async ({ page }) => {
    ...
    // Aguarda resultado
    await expect(page.locator('text=/relatório gerado/i')).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('Integração - Filas (BullMQ)', () => {
  test('deve adicionar job à fila', async ({ page }) => {
    ...
    await expect(page.locator('text=/adicionado à fila/i')).toBeVisible({ timeout: 3000 });
  });
