...
test.describe('Busca - Global (Meilisearch)', () => {
  ...
  test('deve navegar ao clicar em resultado', async ({ page }) => {
    ...
    await page.route('**/indexes/*/search**', async (route) => {
      await route.fulfill({
        status: 200,
        body: JSON.stringify({
          hits: [{ id: '1', title: 'Fornecedor ABC', type: 'fornecedor', slug: 'fornecedor-abc' }],
        }),
      });
    });
  ...
  test('deve exibir "Nenhum resultado"', async ({ page }) => {
    ...
    await expect(page.locator('text=/nenhum resultado|não encontrado/i')).toBeVisible();
  });
});
...
test.describe('Filtros - Listagens', () => {
  ...
  test('deve combinar múltiplos filtros', async ({ page }) => {
    ...
    await expect(page).toHaveURL(/.*categoria=ortopedia.*status=ativo.*busca=parafuso/);
  });
