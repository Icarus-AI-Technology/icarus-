...
export const PedidosCompra: React.FC = () => {
  ...
  const pedidosFiltrados = pedidos.filter((pedido) => {
    ...
    const matchDepartamento =
      ...
      pedido.departamento === filterDepartamento;
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[0.813rem] font-semibold ${bgClass[config.bg]} ${textClass[config.text]}`}
      >
        {config.icon}
        {config.label}
      </span>
          ...
          return (
            <React.Fragment key={index}>
              <div
                ...
                className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-[0.813rem] font-semibold neuro-raised ${
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <span
                  className={`flex items-center gap-1 text-[0.813rem] font-semibold ${trendClass}`}
                >
                  {kpi.trendUp ? (
                    <TrendingUp size={16} />
                  ) : (
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[rgba(99,102,241,0.05)]">
                <th className="p-4 text-left text-[0.813rem] font-semibold text-[var(--orx-text-secondary)] border-b border-black/5">
                  Nº Pedido
                </th>
                <th className="p-4 text-left text-[0.813rem] font-semibold text-[var(--orx-text-secondary)] border-b border-black/5">
                  Fornecedor
                ...
                <th className="p-4 text-center text-[0.813rem] font-semibold text-[var(--orx-text-secondary)] border-b border-black/5">
                  Ações
                </th>
              </tr>
            </thead>
            <tbody>
              {pedidosFiltrados.length === 0 ? (
                <tr>
                  <td
                    colSpan={7}
                    className="p-12 text-center text-[var(--orx-text-secondary)]"
                  >
                      ...
                      <div className="flex items-center justify-center gap-2">
                        ...
                        {(pedido.status === "rascunho" ||
                          ...
                          >
                            <Trash2
                              size={18}
                              className="text-[var(--orx-error-dark)]"
                            />
                          </button>
                        )}
                      </div>
