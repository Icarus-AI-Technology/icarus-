...
export const NotasCompraReformatted: React.FC = () => {
  ...
  const kpis = [
    ...
    },
    {
      ...
      value: notas.filter((n) => n.divergencias && n.divergencias.length > 0)
        .length,
      icon: AlertTriangle,
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        >
          Monitoramento em tempo real com Receita Federal • Validação automática
          SEFAZ • Entrada no estoque
        </p>
      </div>
                  ...
                  <>
                    ...
                    {ultimaVerificacao && (
                      <span>
                        {" "}
                        • Última verificação:{" "}
                        {ultimaVerificacao.toLocaleTimeString("pt-BR")}
                      </span>
                    )}
                  </>
                ) : (
          ...
          >
            <input
              ...
              style={{
                ...
                width: "280px",
                boxShadow:
                  "inset var(--orx-shadow-dark-1), inset var(--orx-shadow-dark-2)",
              }}
            />
            <button
              onClick={() => setMonitoramentoAtivo(!monitoramentoAtivo)}
              disabled={
                !cnpjCliente || cnpjCliente.replace(/[^\d]/g, "").length !== 14
              }
              style={{
                display: "inline-flex",
                alignItems: "center",
                ...
                border: "none",
                cursor:
                  cnpjCliente.replace(/[^\d]/g, "").length === 14
                    ? "pointer"
                    : "not-allowed",
                boxShadow:
                ...
                opacity:
                  cnpjCliente.replace(/[^\d]/g, "").length === 14 ? 1 : 0.5,
              }}
            >
              {monitoramentoAtivo ? (
                <>
                  ...
                  <td style={{ padding: "1rem" }}>
                    ...
                    >
                      ...
                      >
                        <Eye
                          size={18}
                          style={{ color: "var(--orx-info-dark)" }}
                        />
                      </button>
                      <button
                        onClick={() =>
                          handleVerificarSEFAZ(nota.chave_acesso, "SP")
                        }
                        style={{
                          display: "inline-flex",
                          alignItems: "center",
