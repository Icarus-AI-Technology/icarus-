...
class CFMScraperService {
  ...
  async consultarCRM(
    crm: string,
    uf: string,
  ): Promise<CFMScraperResponse | null> {
    ...
    try {
      ...
      const nome = await this.extractText(
        page,
        ".medico-nome, .nome-profissional, h2.nome",
      );
      const situacao = await this.extractText(
        page,
        ".situacao-cadastral, .situacao",
      );
      const tipoCRM = await this.extractText(
        page,
        ".tipo-inscricao, .tipo-crm",
      );
      const dataInscricao = await this.extractText(
        page,
        ".data-inscricao, .data-registro",
      );
      ...
      console.log(
        `[CFM Scraper] ✅ CRM ${crmLimpo}/${ufUpper} encontrado: ${nome}`,
      );
      ...
      return {
        ...
        tipoCRM: tipoCRM?.toUpperCase().includes("SECUNDAR")
          ? "SECUNDARIA"
          : "PRINCIPAL",
