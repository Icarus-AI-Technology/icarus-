...
export class CommunicationService {
  ...
  static async sendSMS(
    ...
    try {
      ...
      const response =
        await APIGatewayService.request<TwilioSMSResponse>(apiRequest);
  ...
  static async sendWhatsApp(
    ...
    try {
      ...
      const response =
        await APIGatewayService.request<WhatsAppMessageResponse>(apiRequest);
        ...
        personalizations: [
          {
            ...
            ...(request.cc && {
              cc: (Array.isArray(request.cc) ? request.cc : [request.cc]).map(
                (email) => ({ email }),
              ),
            }),
            ...(request.bcc && {
              bcc: (Array.isArray(request.bcc)
                ? request.bcc
                : [request.bcc]
              ).map((email) => ({ email })),
            }),
  ...
  static async sendCampaign(
    ...
    try {
      ...
      const createResponse =
        await APIGatewayService.request<MailchimpCampaignResponse>(
          createRequest,
        );
