...
export class ComplianceAI {
  ...
  static async monitorCompliance(empresaId: string): Promise<{
    ...
    try {
      ...
      const areas: ComplianceAreaResumo[] = Array.from(areaMap.entries()).map(
        ([area, areaChecks]) => {
          const areaScore =
            areaChecks.reduce((sum, check) => sum + (check.score || 0), 0) /
            areaChecks.length;
  ...
  static async generateReport(
    ...
    try {
      ...
      const scoreGeral =
        (checks as ComplianceCheckRow[]).reduce(
          (sum, check) => sum + (check.score || 0),
          0,
        ) / checks.length;
      const conformes = (checks as ComplianceCheckRow[]).filter(
        (check) => check.conforme,
      ).length;
      ...
      const scoreFirstHalf =
        (firstHalf as ComplianceCheckRow[]).reduce(
          (sum, check) => sum + (check.score || 0),
          0,
        ) / firstHalf.length;
      const scoreSecondHalf =
        (secondHalf as ComplianceCheckRow[]).reduce(
          (sum, check) => sum + (check.score || 0),
          0,
        ) / secondHalf.length;
  ...
  private static async callAIModel(_prompt: string): Promise<string> {
    ...
    return JSON.stringify({
      ...
      recomendacoes: [
        ...
        "Agendar próxima auditoria em 6 meses",
      ],
      certificacoes: [],
    });
  }
