...
export class CirurgiasAI {
  ...
  async preverDuracaoCirurgia(dados: DadosCirurgia): Promise<PrevisaoDuracao> {
    try {
      ...
      const mediaMedico = this.calcularMedia(
        historicoMedico?.map((c) => c.duracao_real) || [],
      );
      const mediaHospital = this.calcularMedia(
        historicoHospital?.map((c) => c.duracao_real) || [],
      );
      ...
      const totalAmostras =
        (historicoMedico?.length || 0) + (historicoEspecialidade?.length || 0);
      const confianca = Math.min(95, 30 + totalAmostras * 1.5);

      const fatoresConsiderados = [];
      if (historicoMedico && historicoMedico.length > 0) {
        fatoresConsiderados.push(
          `${historicoMedico.length} cirurgias do médico`,
        );
      }
      if (historicoEspecialidade && historicoEspecialidade.length > 0) {
        fatoresConsiderados.push(
          `${historicoEspecialidade.length} cirurgias da especialidade`,
        );
      }
      if (historicoHospital && historicoHospital.length > 0) {
        fatoresConsiderados.push(
          `${historicoHospital.length} cirurgias no hospital`,
        );
      }
  ...
  async recomendarKitCirurgico(
    dados: DadosCirurgia,
  ): Promise<RecomendacaoKit[]> {
    try {
      ...
      for (const cirurgia of cirurgiasSimilares || []) {
        ...
        for (const item of produtos as Array<{
          ...
          const quantidade =
            item.quantidade_consumida ?? item.quantidade_planejada ?? 0;
  ...
  async analisarRiscoCirurgico(dados: DadosCirurgia): Promise<AnaliseRisco> {
    ...
    if (dados.paciente_comorbidades && dados.paciente_comorbidades.length > 0) {
      const impactoComorbidades = Math.min(
        30,
        dados.paciente_comorbidades.length * 10,
      );
  ...
  async preverProbabilidadeGlosas(cirurgiaId: string): Promise<PrevisaoGlosa> {
    try {
      ...
      if (
        !cirurgia?.justificativa_medica ||
        cirurgia.justificativa_medica.length < 50
      ) {
      ...
      if (produtosConsumidos === 0) {
        acoesPreventivas.push(
          "Registrar rastreabilidade de todos os materiais",
        );
      }
  ...
  async otimizarAgendaCirurgica(
    dados: DadosCirurgia,
  ): Promise<OtimizacaoAgenda> {
    try {
      ...
      for (const horario of horariosPossiveis) {
        const score = this.calcularScoreHorario(
          horario,
          cirurgiasNoDia || [],
          duracaoEstimada,
        );
  ...
  async detectarAnomalias(cirurgiaId: string): Promise<AnomaliaDetectada[]> {
    try {
      ...
      if (cirurgia?.duracao_real) {
        const previsao = await this.preverDuracaoCirurgia(
          cirurgia as DadosCirurgia,
        );
        const desvio = Math.abs(
          cirurgia.duracao_real - previsao.duracaoEstimada,
        );
      ...
      if (cirurgia?.valor_materiais) {
        ...
        const mediaPreco = this.calcularMedia(
          historicoPrecos?.map((c) => c.valor_materiais) || [],
        );
        const desvioPercentual =
          ((cirurgia.valor_materiais - mediaPreco) / mediaPreco) * 100;
