...
export const PluggyConnectWidget: React.FC<PluggyConnectWidgetProps> = ({
  ...
  const [step, setStep] = useState<"select" | "credentials" | "connecting">(
    "select",
  );
  ...
  const initializeWidget = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Erro ao inicializar widget";
  ...
  const handlePluggyError = useCallback(
    (error: unknown) => {
      const errorMessage =
        (error as { message?: string })?.message || "Erro ao conectar banco";
      setError(errorMessage);
        ...
        <div style={{ marginBottom: "2rem", textAlign: "center" }}>
          <div
            style={{
              ...
              borderRadius: "50%",
              background:
                "linear-gradient(135deg, var(--orx-primary), var(--orx-secondary))",
              display: "flex",
              alignItems: "center",
              justifyContent: "center",
              margin: "0 auto 1rem",
