    ...
    try {
      ...
      if (lessons) {
        ...
        const progressArray = Array.from(moduleMap.values()).map((p) => ({
          ...p,
          pontuacao_media:
            p.total_licoes > 0 ? p.pontuacao_media / p.total_licoes : 0,
          progresso:
            p.total_licoes > 0 ? (p.concluidas / p.total_licoes) * 100 : 0,
        }));
        ...
        const avgScore =
          lessons.length > 0
            ? lessons.reduce((sum, l) => sum + (l.pontuacao || 0), 0) /
              lessons.length
            : 0;
        const totalTime = lessons.reduce(
          (sum, l) => sum + (l.tempo_gasto || 0),
          0,
        );
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="text-xs opacity-80 mt-1">
            {overallStats.totalLessons > 0
              ? Math.round(
                  (overallStats.completed / overallStats.totalLessons) * 100,
                )
              : 0}
            % de progresso
          </p>
