  ...
  const loadData = async () => {
    try {
      ...
      const [scoreData, fluxoData] = await Promise.all([
        FinanceiroAI.calcularScoreInadimplencia(mockClienteId).catch(
          () => null,
        ),
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-500 mt-2">
                  ...
                  {score.probabilidade_inadimplencia.toFixed(1)}%
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="h-48 flex items-end gap-1">
            {fluxoCaixa.map((dia, idx) => {
              const minSaldo = Math.min(
                ...fluxoCaixa.map((d) => d.saldo_previsto),
              );
              const maxSaldo = Math.max(
                ...fluxoCaixa.map((d) => d.saldo_previsto),
              );
              const altura =
                ((dia.saldo_previsto - minSaldo) / (maxSaldo - minSaldo)) * 100;
                ...
                >
                  ...
                  <div className="absolute -top-20 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white text-xs rounded px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                    <p className="font-medium">
                      {dia.data.split("-").slice(1).join("/")}
                    </p>
                    <p className="text-green-300">
                      ↑ R$ {(dia.entradas_previstas / 1000).toFixed(0)}k
                    </p>
                    <p className="text-red-300">
                      ↓ R$ {(dia.saidas_previstas / 1000).toFixed(0)}k
                    </p>
          ...
          <div className="mt-6 grid grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-green-600">
                R${" "}
                {(
                  fluxoCaixa.reduce((sum, d) => sum + d.entradas_previstas, 0) /
                  1000
                ).toFixed(0)}
                k
              </p>
            ...
            <div className="text-center">
              ...
              <p className="text-2xl font-bold text-red-600">
                R${" "}
                {(
                  fluxoCaixa.reduce((sum, d) => sum + d.saidas_previstas, 0) /
                  1000
                ).toFixed(0)}
                k
              </p>
            ...
            <div className="text-center">
              ...
              <p
                className={`text-2xl font-bold ${fluxoCaixa[fluxoCaixa.length - 1]?.saldo_previsto > 0 ? "text-green-600" : "text-red-600"}`}
              >
                R${" "}
                {(
                  (fluxoCaixa[fluxoCaixa.length - 1]?.saldo_previsto || 0) /
                  1000
                ).toFixed(0)}
                k
              </p>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="text-sm text-blue-800 dark:text-blue-300 mt-2 space-y-1">
              <li>
                • Fluxo de caixa positivo previsto para os próximos 15 dias
              </li>
              <li>
                • Recomenda-se negociar prazos maiores com 3 fornecedores
                principais
              </li>
              <li>
                • Oportunidade de investir R$ 50k em títulos de curto prazo
              </li>
            </ul>
          </div>
        </div>
