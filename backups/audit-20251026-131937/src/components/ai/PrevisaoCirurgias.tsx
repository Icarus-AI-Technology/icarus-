...
import {
  CirurgiasAI,
  PrevisaoDemandaCirurgica,
  AnaliseComplexidade,
} from "@/services/CirurgiasAI";

export function PrevisaoCirurgias() {
  const [loading, setLoading] = useState(true);
  const [demanda, setDemanda] = useState<PrevisaoDemandaCirurgica[]>([]);
  const [complexidade, setComplexidade] = useState<AnaliseComplexidade | null>(
    null,
  );
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-600 dark:text-slate-400">
            Demanda Cirúrgica, Análise de Complexidade, Otimização de
            Agendamento
          </p>
        </div>
        <button
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {demanda.map((item, idx) => (
            <div
              key={idx}
              className="bg-slate-50 dark:bg-slate-900 rounded-xl p-5 space-y-3"
            >
