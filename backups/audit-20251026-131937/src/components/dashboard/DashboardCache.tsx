...
export function DashboardCache() {
  ...
  useEffect(() => {
    ...
    const hitRateMedia =
      stats.reduce((acc, stat) => acc + stat.hit_rate, 0) / stats.length;
    ...
    <div className="space-y-6">
              ...
              {stats.map((stat, index) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-1">
                    <div
                      className={`w-3 h-3 rounded-full ${getColorByType(stat.tipo)}`}
                    />
                    <div>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div className="text-right">
                      ...
                      <div className style={{ fontWeight: 700 }}>
                        {formatNumber(stat.total_consultas)}
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <div className style={{ fontWeight: 700 }}>
                        {stat.consultas_por_dia.toFixed(1)}
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <Card className="orx-card border-primary/20 bg-primary/5">
        ...
        <CardContent className="space-y-2" style={{ fontSize: "0.813rem" }}>
          {hitRateGeral < 50 && (
            <p>
              ⚠️ <strong>Hit rate abaixo de 50%:</strong> Considere aumentar o
              TTL do cache (atualmente 7-30 dias)
            </p>
          )}
          {totalConsultas > 10000 && (
            <p>
              🎉 <strong>Alto volume de consultas:</strong> Cache está
              economizando significativamente em custos de API
            </p>
          )}
          <p>
            ✅ <strong>Dica:</strong> Execute limpeza de cache expirado
            periodicamente usando{" "}
            <code className="bg-secondary px-1 py-0.5 rounded">
              cleanup_validacoes_cache()
            </code>
          </p>
          <p>
            📊 <strong>Análise:</strong> Consultas com hit rate alto indicam
            dados estáveis (CEP, CNPJ)
          </p>
        </CardContent>
      </Card>
    </div>
