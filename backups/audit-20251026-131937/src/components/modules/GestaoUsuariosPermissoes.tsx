...
export default function GestaoUsuariosPermissoes() {
  ...
  const [activeTab, setActiveTab] = useState<
    "usuarios" | "roles" | "permissoes" | "auditoria"
  >("usuarios");
  ...
  const carregarUsuarios = async () => {
    ...
    setUsuarios(
      data.users.map((u) => ({
        ...
        user_metadata: {
          nome: (u.user_metadata as Record<string, unknown>)?.nome as
            | string
            | undefined,
          telefone: (u.user_metadata as Record<string, unknown>)?.telefone as
            | string
            | undefined,
        },
      })),
  ...
  const renderTabUsuarios = () => (
    <div className="space-y-6">
        ...
        <Card className="lg:col-span-1 p-4 neuro-raised">
          ...
          <div className="space-y-2 max-h-[600px] overflow-y-auto">
            {usuariosFiltrados.map((usuario) => (
              ...
              >
                <div className="truncate text-[0.813rem] font-medium">
                  {usuario.email}
                </div>
          ...
          {usuarioSelecionado ? (
            <div className="space-y-6">
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="mb-1 text-[0.813rem] font-semibold">
      ...
      <Dialog
        open={showAtribuirRoleDialog}
        onOpenChange={setShowAtribuirRoleDialog}
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Atribuir Função ao Usuário</DialogTitle>
            <DialogDescription>
              Selecione uma função para atribuir ao usuário{" "}
              {usuarioSelecionado?.email}
            </DialogDescription>
          </DialogHeader>
