...
export default function FinanceiroAvancado() {
  ...
  // Resumos
  const [resumoReceber, setResumoReceber] = useState<ResumoFinanceiro | null>(
    null,
  );
  ...
  const [resumoConciliacao, setResumoConciliacao] =
    useState<ResumoConciliacao | null>(null);

  const calcularResumoFinanceiro = useCallback(
    (
      itens: Array<{ status: string; valor?: number; valor_liquido?: number }>,
    ): ResumoFinanceiro => {
      const total = itens.reduce(
        (acc, item) => acc + (item.valor ?? item.valor_liquido ?? 0),
        0,
      );
      const pagos = itens
      ...
      const inadimplencia =
        total > 0 ? Math.round(((total - pagos) / total) * 100) : 0;
  ...
  const calcularResumoFluxo = useCallback((): ResumoFluxoCaixa | null => {
    ...
    const entradas = fluxoCaixa.fluxoDiario.reduce(
      (acc, dia) => acc + dia.entradas,
      0,
    );
    const saidas = fluxoCaixa.fluxoDiario.reduce(
      (acc, dia) => acc + dia.saidas,
      0,
    );
  ...
  const calcularResumoConciliacao = useCallback((): ResumoConciliacao => {
    ...
    const falhas = conciliacao.extratos.filter(
      (e) => e.status === "falha",
    ).length;
    ...
    return {
      ...
      falhas,
      ultimaAtualizacao:
        conciliacao.extratos[0]?.data || new Date().toISOString(),
    };
  ...
  const submodules: Category[] = useMemo(
    () => [
      ...
      },
      {
        ...
        icon: DollarSign,
        count: contasReceber.contas.filter((c) => c.status === "pendente")
          .length,
      },
      {
        id: "contas_pagar",
  ...
  useEffect(() => {
    ...
    const quantidadeReceber = contasReceber.contas.filter(
      (conta) => conta.status !== "pago",
    ).length;
    const quantidadePagar = contasPagar.contas.filter(
      (conta) => conta.status !== "pago",
    ).length;
    ...
    <div className="space-y-6">
      ...
      <Card className="neuro-raised p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div>
            <p className="text-body-sm text-[var(--text-secondary)] mb-2">
              Total Receitas
            </p>
            <p className="text-heading-lg font-display text-success">
              {formatCurrency(resumoFluxo?.entradas || 0)}
            </p>
          </div>
          <div>
            <p className="text-body-sm text-[var(--text-secondary)] mb-2">
            ...
            <p className="text-heading-lg font-display text-error">
              {formatCurrency(resumoFluxo?.saidas || 0)}
            </p>
          </div>
          <div>
            <p className="text-body-sm text-[var(--text-secondary)] mb-2">
            ...
            <p
              className={`text-heading-lg font-display ${(resumoFluxo?.saldo || 0) >= 0 ? "text-success" : "text-error"}`}
            >
          ...
          <div>
            ...
            <p className="text-heading-lg font-display text-accent">
              {(
                ((resumoFluxo?.saldo || 0) / (resumoFluxo?.entradas || 1)) *
                100
              ).toFixed(1)}
              %
            </p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-body-sm text-[var(--text-secondary)]">
                Total: {formatCurrency(resumoReceber.pendentes)} - Ação
                necessária para reduzir inadimplência
              </p>
            </div>
          </div>
        </Card>
              ...
              <tbody>
                {contasReceber.contas.slice(0, 10).map((conta) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <span
                        className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-body-xs font-medium ${
                          conta.status === "pago"
                            ? "text-success bg-success/5"
                            : conta.status === "vencido"
                              ? "text-error bg-destructive/5"
                    ...
                    <td className="p-4">
                      {conta.score_inadimplencia ? (
                        <span
                          className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-body-xs font-medium ${
                            conta.score_inadimplencia > 70
                              ? "text-error bg-destructive/5"
                              : conta.score_inadimplencia > 40
                                ? "text-warning bg-warning/5"
                                : "text-success bg-success/5"
                          }`}
                        >
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[var(--text-secondary)]">
              Gestão financeira completa com IA, DDA Pluggy e controle em tempo
              real
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {submodules.map((sub) => (
            ...
            >
              ...
              <span className="text-body-xs text-[var(--text-primary)] px-2 font-medium">
                {sub.label}
              </span>
              {sub.count > 0 && (
                <span className="text-body-lg font-display text-[var(--text-primary)] mt-1">
                  {sub.count}
                </span>
              )}
              {sub.badge && (
        ...
        {activeSubmodule === "contas_pagar" &&
          renderPlaceholder("Contas a Pagar", CreditCard)}
        {activeSubmodule === "fluxo_caixa" &&
          renderPlaceholder("Fluxo de Caixa", BarChart3)}
        {activeSubmodule === "conciliacao" &&
          renderPlaceholder("Conciliação Bancária", CheckCircle)}
        {activeSubmodule === "planejamento" &&
          renderPlaceholder("Planejamento Financeiro", CheckCircle)}
        {activeSubmodule === "centro_custos" &&
          renderPlaceholder("Centro de Custos", PieChart)}
        {activeSubmodule === "tesouraria" &&
          renderPlaceholder("Tesouraria", Wallet)}
        {activeSubmodule === "relatorios" &&
          renderPlaceholder("Relatórios Financeiros", FileText)}
        {activeSubmodule === "configuracoes" &&
          renderPlaceholder("Configurações", CheckCircle)}
      </div>
    </div>
  );
}
