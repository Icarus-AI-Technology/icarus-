...
export default function GestaoContabil() {
  ...
  const renderDRE = () => {
    const receitaBruta =
      dreData.find((d) => d.grupo === "RECEITA_BRUTA")?.valor || 0;
    const lucroLiquido =
      dreData.find((d) => d.grupo === "LUCRO_LIQUIDO")?.valor || 0;
    const margemLiquida =
    ...
    const chartData = [
      ...
      {
        name: "Custos",
        value: Math.abs(dreData.find((d) => d.grupo === "CUSTOS")?.valor || 0),
      },
      {
        name: "Desp. Operacionais",
        value: Math.abs(
          dreData.find((d) => d.grupo === "DESPESAS_OP")?.valor || 0,
        ),
      },
              ...
              {dreData.map((item, idx) => {
                ...
                return (
                  <TableRow
                    key={idx}
                    className={
                      isTotal ? "bg-[var(--primary)]/5 font-semibold" : ""
                    }
                  >
          ...
          <TableBody>
            {lancamentos.map((lanc) => (
              <TableRow key={lanc.id}>
                ...
                <TableCell>
                  {new Date(lanc.data_competencia).toLocaleDateString("pt-BR")}
                </TableCell>
