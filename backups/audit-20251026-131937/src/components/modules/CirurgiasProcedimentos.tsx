...
const statusChips: Record<KanbanColumn, string> = {
  ...
  confirmada:
    "bg-[color:var(--brand-blue-100)] text-[color:var(--brand-blue-700)]",
  preparacao: "bg-warning/10 text-warning",
  andamento:
    "bg-[color:var(--brand-indigo-100)] text-[color:var(--brand-indigo-700)]",
  recuperacao:
    "bg-[color:var(--brand-emerald-100)] text-[color:var(--brand-emerald-700)]",
...
export default function CirurgiasProcedimentos(): JSX.Element {
  ...
  const [selectedCirurgiaId, setSelectedCirurgiaId] = useState<string | null>(
    null,
  );
  ...
  const [rastreabilidadeItens, setRastreabilidadeItens] = useState<
    CirurgiaRastreabilidadeRow[]
  >([]);
  const [cirurgiaOcorrencias] =
    useState<OcorrenciaPosOperatorio[]>(initialOcorrencias);
  const [calendarioSemanal, setCalendarioSemanal] = useState<
    CalendarioCirurgiaItem[]
  >([]);
  ...
  const [estatisticasKeywords, setEstatisticasKeywords] = useState<
    EstatisticasPalavraChave[]
  >([]);
  ...
  const [cotacaoRelatorio, setCotacaoRelatorio] =
    useState<RelatorioCotacaoCirurgia | null>(null);
  ...
  const selectedCirurgia = useMemo(() => {
    return selectedCirurgiaId
      ? (cirurgias.find(
          (cirurgiaItem) => cirurgiaItem.id === selectedCirurgiaId,
        ) ?? null)
      : null;
  ...
  const filteredCirurgias = useMemo(() => {
    ...
    return cirurgias.filter((cirurgiaItem) => {
      const tipoProcedimento =
        cirurgiaItem.tipo_procedimento ?? cirurgiaItem.procedimento;
      const matchesSearch =
    ...
    async (cirurgiaId: string) => {
      ...
      try {
        ...
        setRastreabilidadeItens(
          (rastreabilidade.data as CirurgiaRastreabilidadeRow[] | null) ?? [],
        );
            ...
            ? [
                {
                  ...
                  motivo: "Erro reportado pelo portal",
                },
              ]
            : [],
      }));
  ...
  useEffect(() => {
    ...
    const semana: CalendarioCirurgiaItem[] = Array.from(
      { length: 7 },
      (_, index) => {
        ...
        return {
          ...
          cirurgias: cirurgiasDoDia.map((cirurgiaItem) => ({
            id: cirurgiaItem.id,
            titulo:
              cirurgiaItem.tipo_procedimento ??
              cirurgiaItem.procedimento ??
              "Procedimento",
            horario: formatTime(
              cirurgiaItem.hora_inicio ?? cirurgiaItem.hora_cirurgia,
            ),
            hospital:
              hospitais.find(
                (hospitalItem) => hospitalItem.id === cirurgiaItem.hospital_id,
              )?.nome ?? "Hospital",
  ...
  useEffect(() => {
    ...
    const executarIA = async () => {
      try {
        ...
        const [duracao, kit, risco, glosa, agenda, anomalias] =
          await Promise.all([
  ...
  useEffect(() => {
    ...
    const carregarCotacao = async () => {
      try {
        const relatorio = await cotacaoAutomaticaService.cotarPorCirurgia(
          selectedCirurgia.id,
        );
        if (!cancelado) {
  ...
  const handleCreateCirurgia = useCallback(
    async (
      payload: Omit<
        Cirurgia,
        "id" | "created_at" | "updated_at" | "medico" | "hospital"
      >,
    ) => {
      try {
  ...
  const handleDeleteCirurgia = useCallback(
    async (id: string) => {
      const confirmar = window.confirm(
        "Tem certeza que deseja excluir esta cirurgia?",
      );
      if (!confirmar) return;
  ...
  const kpis: KPI[] = useMemo(() => {
    ...
    const cirurgiasHoje = cirurgias.filter(
      (item) => item.data_cirurgia === hojeISO,
    ).length;
    ...
    return [
      ...
      },
      {
        ...
        value:
          total === 0 ? "0%" : `${((concluidas / total) * 100).toFixed(1)}%`,
        ...
        return (
          <Card key={statusKey as string} className="neuro-inset p-4">
            <div className="flex items-center justify-between mb-2">
              <h4 className="text-body-sm text-[var(--text-primary)] font-medium">
                {statusLabel}
              </h4>
  ...
  const navigationItems: NavigationItem[] = useMemo(
    () => [
      ...
      },
      {
        ...
        icon: Clock,
        count: calendarioSemanal.reduce(
          (acc, dia) => acc + dia.cirurgias.length,
          0,
        ),
      },
      {
      ...
      {kpis.map((kpi, index) => {
        ...
        return (
          <div key={index} className="flex items-center gap-3">
            ...
            <div>
              <div className="text-[0.813rem] font-bold text-[var(--orx-text-primary)]">
                {kpi.value}
              </div>
  ...
  const AgendamentoSection = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
      ...
      if (
        !formState.paciente_nome ||
        !formState.tipo_procedimento ||
        !formState.data_cirurgia
      ) {
    ...
    return (
      <Card className="neuro-raised p-6 space-y-6">
        <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
          <div>
            <h3 className="text-body-lg text-[var(--text-primary)] font-medium">
              Agendamento Cirúrgico
            </h3>
            <p className="text-body-sm text-[var(--text-secondary)]">
              Workflow com validações automáticas, previsão de duração via IA e
              integração com estoque OPME.
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <label className="space-y-1">
              ...
              <input
                ...
                onChange={(event) =>
                  setFormState((prev) => ({
                    ...prev,
                    paciente_nome: event.target.value,
                  }))
                }
            ...
            <label className="space-y-1">
              ...
              <input
                ...
                onChange={(event) =>
                  setFormState((prev) => ({
                    ...prev,
                    tipo_procedimento: event.target.value,
                  }))
                }
                className="w-full neuro-inset rounded-xl px-4 py-3 text-body-sm"
            ...
            <label className="space-y-1">
              ...
              <input
                ...
                onChange={(event) =>
                  setFormState((prev) => ({
                    ...prev,
                    data_cirurgia: event.target.value,
                  }))
                }
                className="w-full neuro-inset rounded-xl px-4 py-3 text-body-sm"
                required
            ...
            <label className="space-y-1">
              ...
              <input
                ...
                onChange={(event) =>
                  setFormState((prev) => ({
                    ...prev,
                    numero_cirurgia: event.target.value,
                  }))
                }
                className="w-full neuro-inset rounded-xl px-4 py-3 text-body-sm"
              />
            ...
            <label className="space-y-1">
              ...
              <textarea
                ...
                onChange={(event) =>
                  setFormState((prev) => ({
                    ...prev,
                    justificativa_medica: event.target.value,
                  }))
                }
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            <div className="flex-1 px-4 py-3 rounded-xl neuro-inset text-body-xs text-[var(--text-secondary)]">
              Validação automática CRM/CFM • Autorização ANS pré-preenchida •
              Reserva de sala e equipe sincronizada
            </div>
          </div>
        </form>
      </Card>
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            Controle TISS/ANS com SLA de resposta, alertas de prazos e
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        <Card className="neuro-inset p-4">
          ...
          <p className="text-heading font-display text-[var(--text-primary)]">
            {autorizações.filter((item) => item.status === "aprovada").length}/
            {autorizações.length} aprovadas
          </p>
        </Card>
        <Card className="neuro-inset p-4">
          <p className="text-body-xs text-[var(--text-secondary)]">
        ...
        <Card className="neuro-inset p-4">
          ...
          <p className="text-heading font-display text-[var(--text-primary)]">
            {formatCurrencyBRL(
              autorizações.reduce(
                (acc, item) => acc + (item.valor_solicitado ?? 0),
                0,
              ),
            )}
          </p>
        ...
        <Card className="neuro-inset p-4">
          ...
          <p className="text-heading font-display text-[var(--text-primary)]">
            {formatCurrencyBRL(
              autorizações.reduce(
                (acc, item) => acc + (item.valor_autorizado ?? 0),
                0,
              ),
            )}
          </p>
        ...
        <table className="w-full">
          <thead className="border-b border-[var(--border)]">
            <tr className="text-left">
              <th className="p-4 text-body-xs text-[var(--text-secondary)]">
                Solicitação
              </th>
              <th className="p-4 text-body-xs text-[var(--text-secondary)]">
                Autorização
          ...
          <tbody>
            {autorizações.map((item) => (
              <tr key={item.id} className="border-b border-[var(--border)]">
                ...
                <td className="p-4 text-body-sm text-[var(--text-primary)]">
                  {formatCurrencyBRL(
                    item.valor_autorizado ?? item.valor_solicitado ?? 0,
                  )}
                </td>
                <td className="p-4 text-body-sm text-[var(--text-primary)]">
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            Planejamento inteligente com IA, reserva automática de estoque e
            cotação multiportal.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3">
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {kitItens.map((item) => {
              const sugestao = aiKit.find(
                (kit) => kit.produto_id === item.produto_id,
              );
              return (
                <tr key={item.id} className="border-b border-[var(--border)]">
                  <td className="p-4 text-body-sm text-[var(--text-primary)]">
                  ...
                  <td className="p-4 text-body-sm text-[var(--text-primary)]">
                    {formatCurrencyBRL(
                      item.valor_total ?? item.valor_unitario ?? 0,
                    )}
                  </td>
                  <td className="p-4 text-body-sm text-[var(--text-primary)]">
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            Registro granular com lote, série e validade para cumprimento da RDC
            16/2013.
          </p>
        </div>
        <div className="flex gap-2">
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            Jornada do material registrada ponta a ponta, pronta para auditorias
            ANVISA e ANS.
          </p>
        </div>
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            Checklists de devolução, ocorrências, protocolos clínicos e
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="neuro-inset p-4">
          ...
          <p className="text-heading font-display text-[var(--text-primary)]">
            {kitItens.reduce(
              (acc, item) => acc + (item.quantidade_devolvida ?? 0),
              0,
            )}
          </p>
        </Card>
          ...
          <div key={ocorrencia.id} className="p-4 rounded-xl neuro-inset">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`px-3 py-1 rounded-full text-body-xs font-medium ${severidadeChips[ocorrencia.severidade]}`}
                >
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            Integração Financeiro Avançado, validações TISS, emissão NF-e e
            mitigação de glosas com IA.
          </p>
        </div>
        <div className="flex gap-2">
          <button
      ...
      <section className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="neuro-inset p-4">
          ...
          <p className="text-heading font-display text-[var(--text-primary)]">
            {formatCurrencyBRL(
              kitItens.reduce((acc, item) => acc + (item.valor_total ?? 0), 0),
            )}
          </p>
        </Card>
        <Card className="neuro-inset p-4">
              ...
              {dia.cirurgias.map((item) => (
                ...
                >
                  <div className="flex items-center justify-between text-body-2xs text-[var(--text-secondary)]">
                    ...
                    <span
                      className={`px-2 py-1 rounded-full ${statusChips[item.status]}`}
                    >
                      {statusLabels[item.status]}
                    </span>
                  </div>
                  <p className="text-body-sm text-[var(--text-primary)] mt-1 font-medium">
                    {item.titulo}
                  </p>
                  <p className="text-body-2xs text-[var(--text-secondary)]">
                    {item.hospital}
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            14 indicadores de performance, heatmaps, curva de aprendizado e SLA
            por equipe.
          </p>
        </div>
        <button
      ...
      <div className="rounded-2xl neuro-inset p-6 text-body-sm text-[var(--text-secondary)]">
        Heatmaps por especialidade, curva de aprendizado por equipe cirúrgica,
        índice de ocupação de salas e distribuição por convênios disponíveis no
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex items-start justify-between">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            6 algoritmos proprietários para previsão de duração, recomendação de
            kit, risco cirúrgico, glosas, agenda e detecção de anomalias.
          </p>
        </div>
        <button
          className="neuro-button px-4 py-2 rounded-xl flex items-center gap-2"
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="neuro-inset p-4">
          <h4 className="text-body-sm text-[var(--text-primary)] mb-2 font-medium">
            Sugestão de Kit
          </h4>
          <div className="space-y-2">
            {aiKit.map((item) => (
              ...
              >
                ...
                <p className="text-body-2xs text-[var(--text-secondary)]">
                  Probabilidade {item.probabilidade_uso}% • Base{" "}
                  {item.cirurgias_similares} cirurgias
                </p>
              </div>
            ))}
            {aiKit.length === 0 && (
        ...
        {aiGlosa && (
          <Card className="neuro-inset p-4 space-y-2">
            ...
            <div>
              <p className="text-body-2xs text-[var(--text-secondary)] font-medium">
                Motivos potenciais
              </p>
      ...
      {aiAgenda && (
        <Card className="neuro-inset p-4">
          ...
          <p className="text-body-sm text-[var(--text-primary)]">
            Horário sugerido {aiAgenda.horarioSugerido} • Sala{" "}
            {aiAgenda.salaCircurgica} • Score {aiAgenda.score}
          </p>
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            HL7 v2.x, FHIR R4, TISS, Edge Functions para validações e
            notificações.
          </p>
        </div>
        <span className="px-4 py-2 rounded-xl neuro-inset text-body-xs text-[var(--text-secondary)]">
          12 integrações ativas • 4 Edge Functions • Notificações
          WhatsApp/SMS/E-mail
        </span>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[
          ...
          { title: "TISS ANS", description: "Envio de guias, lotes, SIB" },
          {
            ...
            description: "Validação de estoque, notificações, auditoria",
          },
        ].map((item) => (
    ...
    <Card className="neuro-raised p-6 space-y-6">
      <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
        <div>
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">
            OPMENEXO, Inpart, EMS Ventura e VSSupply integrados com cache
            inteligente, rate limiting e economia média de 15%.
          </p>
        </div>
        <button
          className="neuro-button px-4 py-2 rounded-xl flex items-center gap-2"
          ...
          return (
            <Card key={portal.id} className="neuro-inset p-4">
              ...
              <p className="text-body-2xs text-[var(--text-secondary)] mt-2">
                Última requisição {formatDate(status?.atualizadoEm)} • Latência
                média {(status?.latencia ?? 0).toFixed(1)}s
              </p>
            </Card>
          );
        })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {estatisticasKeywords.slice(0, 6).map((keyword) => (
              <Card key={keyword.palavra_chave} className="neuro-inset p-4">
                ...
                <p className="text-body-2xs text-[var(--text-secondary)]">
                  {keyword.total_buscas} buscas •{" "}
                  {keyword.taxa_sucesso.toFixed(1)}% sucesso
                </p>
                <p className="text-body-2xs text-[var(--text-secondary)]">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        {typeof window !== "undefined" &&
          new URLSearchParams(window.location.search).get("qa") === "1" && (
            <>
              ...
              <div
                role="toolbar"
                aria-label="QA Actions"
                className="flex gap-2 mt-2 flex-nowrap"
              >
                {[
                  "Filtrar",
                  "Exportar",
        ...
        <header className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
          <div>
            ...
            <p className="text-body-sm text-[var(--text-secondary)]">
              Sistema completo com automações, Portais OPME e compliance total.
              ROI 784:1, economia média de 15% por cirurgia.
            </p>
          </div>
          ...
          new URLSearchParams(window.location.search).get("qa") === "1" && (
            <>
              ...
              <div className="neuro-raised p-4 rounded-2xl">
                <h2 className="text-[0.813rem] font-semibold text-[var(--orx-text-primary)] mb-3">
                  Cirurgias (QA)
                </h2>
                <div className="overflow-x-auto">
            ...
            <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-body-xs text-[var(--text-secondary)]">
                    ...
                    {
                      statusLabels[
                        (selectedCirurgia.status as KanbanColumn) ?? "agendada"
                      ]
                    }
                  </p>
                </div>
              ...
              <div className="flex flex-wrap gap-3">
                <button
                  ...
                  className="neuro-button-secondary px-4 py-2 rounded-xl flex items-center gap-2"
                  onClick={() =>
                    handleUpdateStatus(selectedCirurgia.id, "confirmada")
                  }
                >
                  <CheckCircle className="w-4 h-4" />
                  Confirmar
