...
export default function IntegrationsManager() {
  ...
  const carregarIntegracoes = async () => {
    try {
      ...
      const integrationsData: Integration[] = metrics.map(
        (m: Record<string, unknown>) => ({
          ...
          taxa_sucesso:
            Number(m.total_requests ?? 0) > 0
              ? (Number(m.success_count ?? 0) / Number(m.total_requests ?? 0)) *
                100
              : 0,
  ...
  const renderOverview = () => {
    const integracoesAtivas = integrations.filter(
      (i) => i.status === "ativo",
    ).length;
    const integracoesTotal = integrations.length;
    const chamadas24h = integrations.reduce(
      (sum, i) => sum + i.total_chamadas,
      0,
    );
    const taxaSucessoMedia =
      integrations.length > 0
        ? integrations.reduce((sum, i) => sum + i.taxa_sucesso, 0) /
          integrations.length
        : 0;
    const tempoMedio =
      integrations.length > 0
        ? integrations.reduce((sum, i) => sum + i.tempo_resposta_medio, 0) /
          integrations.length
        : 0;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="p-6 neuro-raised bg-gradient-to-br from-emerald-500 to-emerald-600 text-white">
            <div className="flex items-center justify-between mb-3">
              <h3 className="opacity-90 text-[0.813rem] font-medium">
                Integrações Ativas
              </h3>
            ...
            <p className="opacity-80 mt-2 text-[0.813rem]">
              {formatPercent((integracoesAtivas / integracoesTotal) * 100)}{" "}
              operacionais
            </p>
          </Card>
        ...
        <Card className="p-6 neuro-raised">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {integrations.map((integration) => {
              const TypeIcon =
                INTEGRATION_TYPES[
                  integration.tipo as keyof typeof INTEGRATION_TYPES
                ]?.icon || Plug;
            ...
            {integrations.map((integration) => {
              ...
              return (
                <TableRow key={integration.id}>
                  ...
                  <TableCell>
                    {new Date(integration.ultima_sincronizacao).toLocaleString(
                      "pt-BR",
                    )}
                  </TableCell>
                  <TableCell>
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              onClick={() =>
                                handleTestarIntegracao(integration.id)
                              }
                            />
                          </TooltipTrigger>
              ...
              <div>
                ...
                <p className="text-[0.813rem]">
                  {webhook.ultima_chamada
                    ? new Date(webhook.ultima_chamada).toLocaleTimeString(
                        "pt-BR",
                      )
                    : "Nunca"}
                </p>
          ...
          <TableBody>
            {logs.map((log) => (
              <TableRow key={log.id}>
                ...
                <TableCell className="font-mono text-[0.813rem]">
                  {log.endpoint}
                </TableCell>
                <TableCell>
                  <Badge
