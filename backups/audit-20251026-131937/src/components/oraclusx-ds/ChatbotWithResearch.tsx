...
export const ChatbotWithResearch: React.FC<ChatbotWithResearchProps> = ({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (SpeechRecognitionAPI) {
        ...
        console.warn(
          "Web Speech API não disponível neste navegador. Comando por voz desabilitado.",
        );
      }
    }
  }, []);
    ...
    if (isOpen && messages.length === 0) {
      addBotMessage(
        "👋 Olá!\n\n" +
          ...
          "🔗 **NOVO:** Anexe fotos de produtos OPME, embalagens, PDFs com relatos cirúrgicos e prints de tela." +
          "Posso ler documentos e gerar justificativas médicas automaticamente!",
      );
    }
  ...
  useEffect(() => {
    ...
    if (logs.length > 0) {
      ...
      if (
        lastLog.type === "info" &&
        (lastLog.message?.includes("concluída") ?? false)
      ) {
  ...
  const searchLocalKnowledge = async (
    query: string,
  ): Promise<
    Array<{ categoria: string; conteudo_texto: string; modulo?: string }>
  > => {
    try {
  ...
  const searchUserActivities = async (
    userEmail: string,
    days: number = 30,
  ): Promise<
    Array<{
  ...
  const compareUsersForHandover = async (
    userEmailLeaving: string,
    userEmailReplacing: string,
  ): Promise<
    Array<{
      modulo: string;
  ...
  > => {
    try {
      const { data, error } = await supabase.rpc("comparar_usuarios_handover", {
  ...
  const detectUserQuery = (
    query: string,
  ): {
    type: "user_activities" | "user_handover";
    params: Record<string, string>;
  } | null => {
    const lowerQuery = query.toLowerCase();

    // Detectar consulta de atividades: "o que [usuário] fez"
    const activityMatch = lowerQuery.match(
      /o que (?:o |a )?([a-záàâãéèêíïóôõöúçñ\s]+) fez|atividades? (?:do|da) ([a-záàâãéèêíïóôõöúçñ\s]+)/i,
    );
    if (activityMatch) {
    ...
    const handoverMatch = lowerQuery.match(
      /compar(?:ar|ação) (?:entre )?([a-záàâãéèêíïóôõöúçñ\s]+) (?:com|e) ([a-záàâãéèêíïóôõöúçñ\s]+)/i,
    );
    if (handoverMatch) {
...
RESPOSTA:`;
  ...
  const handleSendMessage = async () => {
    ...
    if (userQuery) {
      if (userQuery.type === "user_activities") {
        ...
        addResearchMessage(
          `🔍 Buscando atividades de ${userQuery.params.userName}...`,
        );
        ...
        if (activities.length > 0) {
          const response =
            `📊 **Atividades de ${userQuery.params.userName} (últimos 30 dias):**\n\n` +
            activities
              .map(
                (act, idx) =>
                  `**${idx + 1}. ${act.modulo}**\n` +
          ...
          addBotMessage(
            `❌ Não encontrei atividades para ${userQuery.params.userName}. Verifique se o nome está correto ou se o usuário está cadastrado no sistema.`,
          );
          return;
        }
      }
      ...
      if (userQuery.type === "user_handover") {
        ...
        addResearchMessage(
          `🔄 Comparando ${userQuery.params.user1} com ${userQuery.params.user2}...`,
        );
        ...
        if (comparison.length > 0) {
          ...
          addBotMessage(
            `❌ Não foi possível comparar os usuários. Verifique se os nomes estão corretos.`,
          );
          return;
        }
      }
    ...
    if (localKnowledge.length > 0) {
      ...
      const localResponse = await generateLocalResponse(
        userMessage,
        localKnowledge,
      );
    ...
    if (!isConnected) {
      addBotMessage(
        "❌ GPT Researcher não está conectado. Verifique se o servidor está rodando em" +
  ...
  return (
    <div className={cn("fixed z-50", positionClasses[position], className)}>
      {/* Chat Window */}
      {isOpen && (
        <div className="w-96 h-[600px] mb-4 flex flex-col animate-in slide-in-from-bottom-5 fade-in duration-300 bg-[var(--orx-bg-light)] rounded-[1.25rem] shadow-[var(--orx-shadow-light-1),_var(--orx-shadow-light-2)] border border-white/10">
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b border-b-[rgba(0,0,0,0.05)] rounded-t-[1.25rem]">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pt-4 pr-4 pb-4 pl-0">
            {messages.map((message) => (
              <div
                ...
                className={cn(
                  "flex",
                  message.type === "user"
                    ? "justify-end pr-0 pl-4"
                    : "justify-start pr-4 pl-4",
                )}
              >
                <div
                  className={cn(
                    ...
                    message.type === "user" &&
                      "max-w-[90%] text-white bg-[rgba(99,102,241,0.85)] backdrop-blur-[12px] backdrop-saturate-[180%] border border-white/20 shadow-[4px_4px_8px_rgba(99,102,241,0.2),_-2px_-2px_6px_rgba(255,255,255,0.05),_inset_1px_1px_4px_rgba(0,0,0,0.1)]",
                    message.type === "research" && "bg-[rgba(251,191,36,0.15)]",
                    message.type === "bot" &&
                      "bg-[var(--orx-bg-light)] shadow-[var(--orx-shadow-light-1),_var(--orx-shadow-light-2)]",
                  )}
                >
                  <p className="whitespace-pre-wrap text-[0.813rem] leading-[1.5] m-0">
                    {message.content}
                  </p>
                  {message.sources && message.sources.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-t-[rgba(0,0,0,0.1)]">
                      <p className="mb-1 text-[0.813rem] text-[var(--orx-text-secondary)] font-medium">
                        Fontes:
                      </p>
            ...
            {isResearching && (
              <div className="flex justify-start">
                <div className="rounded-lg p-3 bg-[var(--orx-bg-light)] shadow-[var(--orx-shadow-light-1),_var(--orx-shadow-light-2)]">
                  <Loader2 className="w-5 h-5 animate-spin text-[rgba(99,102,241,1)]" />
                </div>
              </div>
            )}
          ...
          {/* Error Display */}
          {error && (
            <div className="px-4 py-2 bg-[rgba(239,68,68,0.1)] text-[var(--orx-error-dark)] text-[0.813rem]">
              {error}
              <button
                onClick={clearError}
          ...
          {/* Input */}
          <div className="p-4 border-t border-t-[rgba(0,0,0,0.05)]">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className={cn(
                  ...
                  isListening
                    ? "bg-[rgba(239,68,68,0.85)] text-white animate-pulse shadow-[4px_4px_8px_rgba(239,68,68,0.3),_-2px_-2px_6px_rgba(255,255,255,0.05),_inset_1px_1px_4px_rgba(0,0,0,0.1),_0_0_20px_rgba(239,68,68,0.4)]"
          ...
          <div className="px-4 pb-3 pt-2 border-t border-t-[rgba(229,231,235,0.5)]">
      ...
      {!isOpen && (
        <div className="absolute mb-2 right-0 bottom-[105px] bg-[rgba(255,255,255,0.7)] backdrop-blur-[12px] px-5 py-3 rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.1)] whitespace-nowrap border border-white/30">
      ...
      >
        ...
        <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[20px] h-[20px] px-1 text-white rounded-full bg-[var(--orx-error)] text-[0.813rem] shadow-[0_2px_4px_rgba(239,68,68,0.3)] font-bold">
          3
        </span>
      </button>
    </div>
