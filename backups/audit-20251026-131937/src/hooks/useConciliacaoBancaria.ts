...
export function useConciliacaoBancaria() {
  ...
  const importarOFX = useCallback(
    async (arquivoOFX: string, bancoId: string) => {
      try {
        ...
        const err = error as Error;
        const message =
          err instanceof Error ? err.message : "Erro ao importar OFX";
        setError(message);
  ...
  const buscarSugestoes = useCallback(
    async (extratoId: string): Promise<SugestãoConciliação[]> => {
      try {
        const { conciliacaoBancariaService } = await import(
          "@/lib/services/ConciliacaoBancariaService"
        );
        ...
        const sugestoes =
          await conciliacaoBancariaService.buscarSugestoesConciliacao(
  ...
  const conciliarManual = useCallback(
    async (
      ...
      try {
        ...
        const tabelaConta =
          tipoConta === "receber" ? "contas_receber" : "contas_pagar";
        await supabase
          .from(tabelaConta)
          .update({ status: "pago" })
        ...
        const err = error as Error;
        const message =
          err instanceof Error ? err.message : "Erro ao conciliar manualmente";
        setError(message);
    ...
    async (scoreMinimo: number = 90) => {
      try {
        ...
        for (const extrato of extratosNaoConciliados) {
          ...
          const melhorSugestao = sugestoes.find(
            (s) => s.match_score >= scoreMinimo,
          );
          if (melhorSugestao) {
        ...
        const err = error as Error;
        const message =
          err instanceof Error ? err.message : "Erro na conciliação automática";
        setError(message);
  ...
  const desconciliar = useCallback(async (extratoId: string) => {
    try {
      ...
      const err = error as Error;
      const message =
        err instanceof Error ? err.message : "Erro ao desconciliar";
      setError(message);
  ...
  const getResumo = useCallback(async (): Promise<ConciliacaoResumo> => {
    try {
      ...
      const valorTotal = extratosData.reduce(
        (sum, e) => sum + Math.abs(e.valor),
        0,
      );
      const valorConciliado = conciliados.reduce(
        (sum, e) => sum + Math.abs(e.valor),
        0,
      );
      ...
      const mediaScore =
        conciliados.length > 0
          ? conciliados.reduce((sum, e) => sum + (e.match_score || 0), 0) /
            conciliados.length
          : 0;
      ...
      return {
        ...
        percentual_conciliado:
          total > 0 ? (conciliados.length / total) * 100 : 0,
  ...
  const conectarPluggy = useCallback(async (bancoId: string) => {
    try {
      ...
      const err = error as Error;
      const message =
        err instanceof Error ? err.message : "Erro ao conectar Pluggy";
      setError(message);
