...
export function useContasPagar() {
  ...
  const updateConta = useCallback(
    async (id: string, updates: Partial<ContaPagar>) => {
      try {
        setError(null);
  ...
  // Aprovar conta
  const aprovarConta = useCallback(
    async (
      id: string,
      nivel: 1 | 2 | 3,
      aprovadorId: string,
      ...
      try {
        ...
        const err = error as Error;
        const message =
          err instanceof Error ? err.message : "Erro ao aprovar conta";
        setError(message);
  ...
  // Agendar pagamento
  const agendarPagamento = useCallback(
    async (
      id: string,
      ...
      try {
        ...
        const err = error as Error;
        const message =
          err instanceof Error ? err.message : "Erro ao agendar pagamento";
        setError(message);
  ...
  const pagarConta = useCallback(
    async (
      ...
      try {
        ...
        const err = error as Error;
        const message =
          err instanceof Error ? err.message : "Erro ao pagar conta";
        setError(message);
  ...
  const getResumo = useCallback(async () => {
    try {
      setError(null);
      const { data, error: resumoError } = await supabase.rpc(
        "contas_pagar_resumo",
      );
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Erro ao carregar resumo de contas a pagar";
      setError(message);
      ...
      const vencido = contas
        ...
        .reduce((acc, conta) => acc + conta.valor_liquido, 0);
