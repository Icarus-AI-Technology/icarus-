    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-[var(--orx-text-primary)]">
  ...
  return (
    <div className="space-y-6 p-6">
        ...
        new URLSearchParams(window.location.search).get("qa") === "1" && (
          ...
          >
            ...
            <div className="[grid-column:1/-1] flex gap-2">
              <button
                type="submit"
                className="neumorphic-button px-3 py-2 rounded-xl"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MiniCard
          title="Total Médicos"
          value={kpis.medicos?.toLocaleString?.() ?? "—"}
        />
        <MiniCard
          title="Hospitais"
          value={kpis.hospitais?.toLocaleString?.() ?? "—"}
        />
        <MiniCard
          title="Fornecedores"
          value={kpis.fornecedores?.toLocaleString?.() ?? "—"}
        />
        <MiniCard
          title="Produtos OPME"
          value={kpis.produtos?.toLocaleString?.() ?? "—"}
        />
      </div>
        ...
        <div className="neumorphic-container p-6 rounded-2xl">
          ...
          <div className="space-y-3">
            {alertas.map((alerta) => (
              <div
                key={alerta.id}
                className="neumorphic-container p-4 rounded-xl flex items-start gap-3"
              >
                <AlertTriangle
                  size={20}
        ...
        <div className="neumorphic-container p-6 rounded-2xl">
          ...
          <div className="space-y-3">
            {duplicatas.map((duplicata) => (
              <div
                key={duplicata.id}
                className="neumorphic-container p-4 rounded-xl flex items-center justify-between"
              >
                <div>
                  <h3 className="font-semibold text-[var(--orx-text-primary)] mb-1">
                    {duplicata.nome}
