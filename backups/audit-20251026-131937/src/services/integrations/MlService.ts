...
const runtimeImportMeta: EnvBag | undefined =
  typeof import.meta !== "undefined"
    ? (import.meta as unknown as EnvBag)
    : undefined;
...
function resolveVectorEndpoint(): string | undefined {
  const fromImportMeta = runtimeImportMeta?.env?.VITE_ML_VECTOR_URL as
    | string
    | undefined;
...
export async function optimizeObjective(objective: number[]) {
  const { data } = await axios.post(`${ML_API_URL}/optimizer/or-tools`, {
    objective,
  });
  return data;
}
