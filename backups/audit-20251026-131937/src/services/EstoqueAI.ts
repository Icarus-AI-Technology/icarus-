...
export class EstoqueAI {
  ...
  static async preverDemanda(produtoId: string): Promise<PrevisaoDemanda> {
    try {
      ...
      if (demandaMensal.length >= 3) {
        const primeiros =
          demandaMensal.slice(0, 3).reduce((a, b) => a + b, 0) / 3;
      ...
      return {
        ...
        demanda_prevista_30_dias: Math.round(
          mediaMovelPonderada * fatorCrescimento,
        ),
    ...
    try {
      ...
      for (const prod of produtos || []) {
        ...
        switch (classe) {
          case "AX":
            estrategia =
              "Controle rigoroso, estoque mínimo, reposição frequente";
            break;
          case "AY":
  ...
  static async calcularEOQ(produtoId: string): Promise<CalculoEOQ> {
    try {
      ...
      const custoArmazenagem =
        (produto.custo_medio || 0) * custoArmazenagemPerc;

      // Fórmula EOQ
      const eoq = Math.sqrt(
        (2 * demandaAnual * custoPedido) / custoArmazenagem,
      );
      const numeroPedidosAno = demandaAnual / eoq;
      const custoTotalAnual =
        numeroPedidosAno * custoPedido + (eoq / 2) * custoArmazenagem;
  ...
  static async detectarAnomalias(): Promise<Anomalia[]> {
    try {
      ...
      const { data: semMovimento } = await supabase.rpc(
        ...
        if (pontoReposicao > 0 && qtdAtual > pontoReposicao * 3) {
          anomalias.push({
            ...
            desvio_percentual: Math.round(
              ((qtdAtual - pontoReposicao) / pontoReposicao) * 100,
            ),
  ...
  static async otimizarEstoque(): Promise<OtimizacaoEstoque[]> {
    try {
      ...
      for (const prod of produtos || []) {
        ...
        const demandaMensal =
          (movimentacoes?.reduce((s, m) => s + m.quantidade, 0) || 0) / 3;
  ...
  private static detectarSazonalidade(demandaMensal: number[]): boolean {
    ...
    const media =
      demandaMensal.reduce((a, b) => a + b, 0) / demandaMensal.length;
    const variancia =
      demandaMensal.reduce((sum, val) => sum + Math.pow(val - media, 2), 0) /
