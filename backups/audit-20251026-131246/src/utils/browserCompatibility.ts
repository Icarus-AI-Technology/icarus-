...
export const BrowserDetection = {
  ...
  isSafari: () => {
    const isSafari =
      /constructor/i.test(
        String((window as unknown as { HTMLElement: unknown }).HTMLElement),
      ) ||
      ((p: unknown): boolean => {
        return String(p) === "[object SafariRemoteNotification]";
      })(
        !(window as unknown as { safari?: { pushNotification?: unknown } })
          .safari ||
          (typeof (
            window as unknown as { safari?: { pushNotification?: unknown } }
          ).safari !== "undefined" &&
...
export const initIntersectionObserver = () => {
  ...
  class BasicIntersectionObserver {
    private _callback: (
      entries: unknown[],
      observer: BasicIntersectionObserver,
    ) => void;
    constructor(
      callback: (
        entries: unknown[],
  ...
  ).IntersectionObserver = BasicIntersectionObserver as unknown;
};
...
export const polyfillSmoothScroll = () => {
  ...
  // Override scrollTo
  const originalScrollTo = Element.prototype.scrollTo as (
    x: number,
    y: number,
  ) => void;
...
export const initBrowserCompatibility = () => {
  ...
  console.log(
    `🌐 Navegador detectado: ${browserInfo.name} ${browserInfo.version}`,
  );
