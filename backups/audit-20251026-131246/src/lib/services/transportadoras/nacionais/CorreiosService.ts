...
export class CorreiosService implements TransportadoraService {
  ...
  async cotarFrete(params: CotacaoParams): Promise<CotacaoResult[]> {
    ...
    const promises = servicosDisponiveis.map((codServico) =>
      this.calcularPrecoPrazo({
        ...
        valorDeclarado:
          ...
          volumes.reduce((sum, v) => sum + (v.valor || 0), 0),
      }),
    );
    ...
    return resultados
      .filter(
        (
          resultado,
        ): resultado is PromiseFulfilledResult<CotacaoResult | null> =>
  ...
  private async calcularPrecoPrazo(params: {
    ...
    try {
      ...
      console.error(
        `Erro ao cotar Correios ${params.codServico}:`,
        err.message ?? err,
      );
      return null;
    }
  ...
  private getMockCotacao(params: {
    ...
    const servico =
      this.SERVICOS[params.codServico as keyof typeof this.SERVICOS];
