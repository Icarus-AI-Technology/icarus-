...
export class TransportadorasAPIGateway {
  ...
  async cotarMultiplas(
    ...
    const promises = transportadoras.map(async (service) => {
      try {
        ...
        console.error(
          `Erro em ${service.constructor.name}:`,
          err.message ?? err,
        );
        return [];
      }
    ...
    const cotacoesValidas = resultados
      .filter(
        (r): r is PromiseFulfilledResult<CotacaoResult[]> =>
          r.status === "fulfilled",
      )
      .flatMap((r) => r.value);
  ...
  async agendarColeta(
    ...
    if (!service.agendarColeta) {
      throw new Error(
        "Transportadora não suporta agendamento de coleta via API",
      );
    }
  ...
  private async executeWithProtection<T>(
    ...
    if (breaker.isOpen) {
      const timeSinceLastFailure =
        Date.now() - (breaker.lastFailure?.getTime() || 0);
    ...
    if (breaker.failures >= this.CIRCUIT_BREAKER_THRESHOLD) {
      breaker.isOpen = true;
      console.error(
        `Circuit breaker ABERTO para ${serviceName} após ${breaker.failures} falhas`,
      );
    }
