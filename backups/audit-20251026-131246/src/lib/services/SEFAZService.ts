...
export class SEFAZService {
  ...
  async emitirNFe(dados: DadosNFe): Promise<ResultadoNFe> {
    try {
      ...
      // 5. Enviar para SEFAZ
      const resultado = await this.enviarParaSEFAZ(
        xmlAssinado,
        dados.emitente.endereco.uf,
      );

      if (resultado.sucesso) {
        // 6. Gerar DANFE (PDF)
        const danfePdf = await this.gerarDANFE(
          xmlAssinado,
  ...
  async cancelarNFe(
    ...
    try {
      ...
      const diferencaHoras =
        (agora.getTime() - dataAutorizacao.getTime()) / (1000 * 60 * 60);
      ...
      const xmlCancelamento = this.montarXMLCancelamento(
        chaveAcesso,
        protocolo,
        justificativa,
      );
  ...
  private validarDadosNFe(dados: DadosNFe): {
    valido: boolean;
    erros: string[];
  } {
    const erros: string[] = [];
  ...
  private gerarChaveAcesso(dados: DadosNFe): string {
    ...
    const aamm = new Date(dados.data_emissao)
      .toISOString()
      .slice(2, 7)
      .replace("-", "");
...
<envEvento xmlns="http://www.portalfiscal.inf.br/nfe">
  ...
  private async enviarCancelamentoSEFAZ(
    _xml: string,
    _uf: string,
  ): Promise<ResultadoNFe> {
    // Mock
