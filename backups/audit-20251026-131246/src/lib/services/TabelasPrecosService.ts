...
class TabelasPrecosService {
  ...
  async obterMelhorPreco(params: {
    ...
    try {
      const { data, error } = await supabase.rpc("obter_melhor_preco", {
        ...
        p_empresa_id: (await supabase.auth.getUser()).data.user?.user_metadata
          ?.empresa_id,
  ...
  async duplicarTabela(id: string, novoNome: string) {
    try {
      ...
      const { data: tabelaOriginal, error: errorTabela } =
        await this.buscarTabela(id);
      if (errorTabela || !tabelaOriginal) {
        return {
          data: null,
          error: errorTabela || new Error("Tabela não encontrada"),
        };
      }

      // Buscar itens da tabela original
      const { data: itensOriginais, error: errorItens } =
        await this.listarItens(id);
      if (errorItens) {
      ...
      const { data: novaTabela, error: errorNova } = await this.criarTabela({
        nome: novoNome,
        codigo: tabelaOriginal.codigo
          ? `${tabelaOriginal.codigo}-COPIA`
          : undefined,
      ...
      if (errorNova || !novaTabela) {
        return {
          data: null,
          error: errorNova || new Error("Erro ao criar nova tabela"),
        };
      }
  ...
  async aplicarReajuste(
    ...
    try {
      ...
      const { data: itens, error: errorItens } = await this.listarItens(
        tabelaPrecoId,
        { ativo: true },
      );
      if (errorItens || !itens) {
