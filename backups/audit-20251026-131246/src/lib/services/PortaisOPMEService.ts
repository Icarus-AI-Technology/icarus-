...
export class PortaisOPMEService {
    ...
    try {
      ...
      const economiaEstimada =
        melhorOferta && params.precoCustoAtual
          ? (params.precoCustoAtual - melhorOferta.preco_unitario) *
            params.quantidade
          : 0;

      const percentualEconomia =
        economiaEstimada > 0 && params.precoCustoAtual
          ? (economiaEstimada / (params.precoCustoAtual * params.quantidade)) *
            100
          : 0;
  ...
  private async cotarPortal(
    ...
    try {
      ...
      // Salvar em cache
      this.salvarCache(
        { portal: portal.portal, palavraChave, quantidade },
        ofertas,
      );
  ...
  private calcularScore(oferta: OfertaOPME): number {
    ...
    // Portais premium (+5)
    if (oferta.portal === "opmenexo" || oferta.portal === "vssupply")
      score += 5;
  ...
  private async buscarPortaisAtivos(
    portaisFiltro?: string[],
  ...
  private async atualizarEstatisticasPortal(
    portal: string,
    sucesso: boolean,
  ): Promise<void> {
