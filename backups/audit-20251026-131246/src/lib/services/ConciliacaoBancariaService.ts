...
export class ConciliacaoBancariaService {
  ...
  async parseOFX(conteudoOFX: string): Promise<Partial<ExtratoBancario>[]> {
    try {
      ...
      for (const match of transacoes) {
        ...
        const historico =
          this.extrairCampo(bloco, "MEMO") || this.extrairCampo(bloco, "NAME");
        const documento =
          this.extrairCampo(bloco, "CHECKNUM") ||
          this.extrairCampo(bloco, "REFNUM");
   ...
   */
  async buscarSugestoesConciliacao(
    extrato: ExtratoBancario,
  ): Promise<SugestãoConciliação[]> {
    try {
      ...
      const { data: contasReceber } = await supabase
        .from("contas_receber")
        .select(
          "id, numero_documento, cliente_nome, valor_original, data_vencimento, status",
        )
      ...
      const { data: contasPagar } = await supabase
        .from("contas_pagar")
        .select(
          "id, numero_documento, fornecedor_nome, valor_original, data_vencimento, status",
        )
  ...
  private calcularScoreMatch(
    ...
    if (conta.data_vencimento) {
      ...
      const diferencaDias = Math.abs(
        Math.floor(
          (dataExtrato.getTime() - dataVencimento.getTime()) /
            (1000 * 60 * 60 * 24),
        ),
      );
      if (diferencaDias <= 30) {
    ...
    if (nomeNormalizado.length > 0) {
      ...
      const distancia = this.levenshteinDistance(
        historicoExtrato,
        nomeNormalizado,
      );
