...
export class CotacaoAutomaticaService {
  ...
  async cotarPorCirurgia(
    cirurgiaId: string,
  ): Promise<RelatorioCotacaoCirurgia> {
    ...
    try {
      ...
      for (const item of produtos) {
        ...
        const melhorPreco =
          resultadoCotacao.melhorOferta?.preco_unitario || precoAtual;
        const economia = Math.max(
          0,
          (precoAtual - melhorPreco) * item.quantidade_planejada,
        );
        const percentualEconomia =
          precoAtual > 0
            ? (economia / (precoAtual * item.quantidade_planejada)) * 100
            : 0;
        ...
        cotacoes.push({
          ...
          fornecedor_recomendado:
            resultadoCotacao.melhorOferta?.fornecedor || "",
      ...
      const percentualEconomia =
        valorTotalAtual > 0 ? (economiaTotal / valorTotalAtual) * 100 : 0;
  ...
  async cotarMultiplasCirurgias(cirurgiasIds: string[]): Promise<{
    ...
    const economiamedia =
      cirurgiasIds.length > 0 ? economiaTotal / cirurgiasIds.length : 0;
  ...
  async gerarRankingFornecedores(
    cirurgiaId: string,
  ): Promise<RankingFornecedores[]> {
    ...
    for (const cotacao of relatorio.cotacoes) {
      for (const oferta of cotacao.top3_ofertas) {
        ...
        if (fornecedoresMap.has(chave)) {
          ...
          ranking.score_medio =
            (ranking.score_medio + oferta.score_qualidade) / 2;
  ...
  async gerarEstatisticas(
    ...
    const totalCirurgias = new Set(
      cotacoes?.map((c) => c.cirurgia_id).filter(Boolean),
    ).size;
    const totalProdutos = cotacoes?.length || 0;
    const economiaTotal =
      cotacoes?.reduce((sum, c) => sum + (c.economia_estimada || 0), 0) || 0;
    const economiaMedia =
      totalCirurgias > 0 ? economiaTotal / totalCirurgias : 0;

    // Calcular percentual médio
    const cotacoesComEconomia =
      cotacoes?.filter((c) => c.percentual_economia > 0) || [];
    const percentualMedio =
      cotacoesComEconomia.length > 0
        ? cotacoesComEconomia.reduce(
            (sum, c) => sum + (c.percentual_economia || 0),
            0,
    ...
    for (const cotacao of cotacoes || []) {
      if (cotacao.portal_melhor_preco && cotacao.economia_estimada) {
        ...
        portaisEconomia.set(
          cotacao.portal_melhor_preco,
          atual + cotacao.economia_estimada,
        );
      }
    }
  ...
  private gerarRankingPortais(cotacoes: CotacaoProduto[]): {
    ...
    for (const cotacao of cotacoes) {
      if (cotacao.portal_recomendado) {
        const atual = portaisMap.get(cotacao.portal_recomendado) || {
          total: 0,
          economia: 0,
        };
        atual.total++;
  ...
  async exportarRelatorioCSV(
    relatorio: RelatorioCotacaoCirurgia,
  ): Promise<string> {
    const linhas: string[] = [];
