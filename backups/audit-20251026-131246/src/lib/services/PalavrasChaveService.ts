...
export class PalavrasChaveService {
  ...
  async atualizarPalavraChave(
    id: string,
    updates: Partial<PalavraChave>,
  ): Promise<PalavraChave> {
    const { data, error } = await supabase
      .from("portais_opme_palavras_chave")
      .update(updates)
  ...
  async sugerirPalavrasChaveComIA(
    produto: ProdutoParaSugestao,
  ): Promise<SugestaoIA[]> {
    try {
...
`.trim();
  ...
  async analisarEfetividade(
    produtoId: string,
  ): Promise<EstatisticasPalavraChave[]> {
    ...
    const estatisticas: EstatisticasPalavraChave[] = palavrasChave.map((pc) => {
      const mediaOfertas =
        pc.total_buscas > 0 ? pc.total_resultados / pc.total_buscas : 0;
  ...
  async sincronizarComPortais(produtoId: string): Promise<{
    ...
    const termosExistentes = new Set(
      (await this.buscarPalavrasChave({ produtoId })).map((pc) =>
        pc.palavra_chave.toLowerCase(),
      ),
    );
    ...
    for (const cotacao of cotacoes || []) {
      ...
      if (
        !termosExistentes.has(termo) &&
        cotacao.total_ofertas_encontradas > 0
      ) {
  ...
  async gerarRelatorioGeral(): Promise<{
    ...
    const taxaSucessoMedia =
      palavrasChave.reduce((sum, pc) => sum + pc.taxa_sucesso, 0) /
