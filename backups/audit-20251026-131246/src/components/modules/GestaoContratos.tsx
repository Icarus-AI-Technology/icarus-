...
export default function GestaoContratos() {
  ...
  const [clausulas, setClausulas] = useState<
    Awaited<ReturnType<typeof getClausulas>>
  >([]);
  const [aditivos, setAditivos] = useState<
    Awaited<ReturnType<typeof getAditivos>>
  >([]);
  const [slas, setSlas] = useState<Awaited<ReturnType<typeof getSLAs>>>([]);
  const [aprovacoes, setAprovacoes] = useState<
    Awaited<ReturnType<typeof getAprovacoes>>
  >([]);
  ...
  const contratosVencendo = useMemo(() => {
    ...
    return contratosOrdenados.filter((contrato) => {
      ...
      return (
        isBefore(vencimento, addDays(hoje, 30)) && contrato.status === "ativo"
      );
    });
  }, [contratosOrdenados]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {dashboardData.map((card) => (
          <Card key={card.id} className="neuro-raised h-full">
            ...
            <CardContent>
              <p className="text-heading text-[var(--text-primary)] font-display">
                {card.value}
              </p>
              {card.secondary && (
    ...
    <Table
      columns={[
        ...
        },
        {
          ...
          accessor: "tipo",
          cell: ({ row }) => (
            <Badge variant="default">{row.tipo.replaceAll("_", "")}</Badge>
          ),
        },
        {
          header: "Ações",
          ...
          cell: ({ row }) => (
            <div className="flex gap-2">
              <Button
                variant="default"
                size="sm"
                onClick={() => handleSelectContrato(row.id)}
              >
      ...
      data={contratosOrdenados.filter(
        (contrato) =>
          contrato.status === "ativo" || contrato.status === "renovacao",
      )}
      emptyState={{
        icon: FileText,
        ...
        action: {
          label: "Novo contrato",
          onClick: () =>
            addToast("Fluxo de criação em desenvolvimento", "info"),
        },
      }}
    />
  );
          ...
          <CardContent className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-4">
            <div>
              ...
              <p className="text-body-xs text-[var(--text-secondary)]">
                ...
                {formatterCurrency.format(contrato.valor_total ?? 0)}
              </p>
            </div>
            ...
            <Card className="neuro-raised">
              <CardHeader className="flex-row items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Etapas e responsáveis pela aprovação
                  </CardDescription>
                </div>
                <Badge variant="info">
                  Contrato #{selectedContratoId.slice(0, 6)}
                </Badge>
              </CardHeader>
        ...
        <Card className="neuro-raised">
          <CardContent className="py-12 text-center">
            <p className="text-body-sm text-[var(--text-secondary)] mb-4">
              Selecione um contrato na lista para visualizar o workflow de
              aprovação.
            </p>
    ...
    <div className="grid gap-6">
      <Card className="neuro-raised">
        <CardHeader>
          ...
          <CardDescription>
            Texto contratual armazenado no banco
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <Table
              columns={[
                ...
                },
                {
                  ...
                  cell: ({ row }) =>
                    formatterCurrency.format(row.valor_ajuste ?? 0),
                },
              ]}
              data={aditivos}
            />
  ...
  const renderSLA = () => (
    <Card className="neuro-raised">
      <CardHeader>
        ...
        <CardDescription>
          Metas e penalidades associadas ao contrato
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <Table
          columns={[
            ...
            },
            {
              ...
              cell: ({ row }) =>
                row.prazo_aviso_rescisao
                  ? `${row.prazo_aviso_rescisao} dias`
                  : "—",
            },
          ]}
          data={contratosOrdenados.filter(
            (contrato) => contrato.status === "renovacao",
          )}
          emptyState={{
            icon: RefreshCw,
            title: "Nenhuma renovação",
            description:
              "Os contratos ativos serão listados aqui quando entrarem em renegociação.",
          }}
        />
      </CardContent>
    </Card>
    ...
    <Card className="neuro-raised">
      <CardHeader className="flex-row items-center justify-between">
        <div>
          ...
          <CardDescription>
            Recomendações contextuais ContratosAI
          </CardDescription>
        </div>
        <Button
          variant="default"
          ...
          onClick={() =>
            selectedContratoId && handleExecutarAnaliseIA(selectedContratoId)
          }
        >
  ...
  const renderRelatorios = () => (
    <Card className="neuro-raised">
      <CardHeader>
        ...
        <CardDescription>
          Exportação de dados para BI corporativo
        </CardDescription>
      </CardHeader>
          ...
          <Card key={item.id} className="neuro-inset">
            <CardContent className="pt-6">
              ...
              <Button
                ...
                className="mt-4"
                onClick={() =>
                  addToast("Exportação em desenvolvimento", "info")
                }
              >
        ...
        <header className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-body-sm text-[var(--text-secondary)] mt-2">
              Controle completo de contratos de fornecimento, OPME, SLA e
              renovações com IA e alertas proativos.
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="default" size="sm">
