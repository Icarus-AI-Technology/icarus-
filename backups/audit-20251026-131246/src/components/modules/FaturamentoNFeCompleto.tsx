...
export default function FaturamentoNFeCompleto() {
  ...
  const [filterStatus, setFilterStatus] = useState<NFe["status"] | "todos">(
    "todos",
  );
  ...
  const loadKPIs = useCallback(async () => {
    try {
      ...
      const valor_total_mes =
        data?.reduce((sum, nfe) => sum + nfe.valor_total, 0) || 0;
      const autorizadas =
        data?.filter((nfe) => nfe.status === "autorizada").length || 0;
      const rejeitadas =
        data?.filter((nfe) => nfe.status === "rejeitada").length || 0;
      const taxa_autorizacao =
        total_mes > 0 ? (autorizadas / total_mes) * 100 : 0;

      // Conformidade ANVISA (média de % de produtos rastreados)
      const conformidade_anvisa =
        data?.reduce(
          (sum, nfe) =>
            sum + (nfe.rastreabilidade_anvisa?.percentual_conformidade || 0),
          0,
        ) / (total_mes || 1);
  ...
  const emitirNFe = async (nfeData: Partial<NFe>) => {
    ...
    try {
      ...
      const valor_produtos = nfeData.produtos.reduce(
      ...
      const { data: resultadoSEFAZ, error: erroSEFAZ } =
        await supabase.functions.invoke("emitir-nfe-sefaz", {
  ...
  const cancelarNFe = async (nfeId: string, motivo: string) => {
    if (!motivo || motivo.length < 15) {
      addToast(
        "Motivo de cancelamento deve ter no mínimo 15 caracteres",
        "error",
      );
      return;
    }
    ...
    if (
      !confirm(
        "Confirma o cancelamento desta NF-e? Esta ação não pode ser desfeita.",
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const { data: resultadoCancelamento, error } =
        await supabase.functions.invoke("cancelar-nfe-sefaz", {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="kpi-card bg-[var(--orx-primary)] text-white h-[140px] p-6">
          <div className="flex items-start justify-between h-full">
            ...
            <div className="flex-1 ml-4">
              <p className="text-white/80 mb-1 text-[0.813rem]">
                Taxa Autorização
              </p>
              <p className="text-white text-[0.813rem] font-bold">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1 text-yellow-700 dark:text-yellow-300 text-[0.813rem]">
                {kpis.conformidade_anvisa < 95 && (
                  <li>
                    • Rastreabilidade ANVISA abaixo de 95%: Verifique lotes e
                    validades
                  </li>
                )}
                {kpis.rejeitadas > 5 && (
                  <li>
                    • {kpis.rejeitadas} NF-es rejeitadas este mês: Revise
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <div className="text-[var(--text-secondary)] text-[0.813rem]">
                      {nfe.rastreabilidade_anvisa.percentual_conformidade.toFixed(
                        0,
                      )}
                      % ANVISA
                    </div>
      ...
      <Card className="p-4">
        <div className="flex items-center gap-4">
          ...
          <select
            value={filterStatus}
            onChange={(e) =>
              setFilterStatus((e.target.value as NFe["status"]) || "todos")
            }
  ...
  const renderRastreabilidade = () => (
    <Card className="p-6">
      <h2 className="mb-4 text-[0.813rem] font-semibold">
      ...
      <div className="space-y-4">
        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <p className="text-blue-700 dark:text-blue-300 text-[0.813rem]">
            Todos os dispositivos médicos (OPME) comercializados devem ter
            rastreabilidade completa: número de registro ANVISA, lote, data de
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 neuro-flat rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-medium text-[0.813rem]">
                Produtos Rastreados
              </span>
            </div>
          ...
          <div className="p-4 neuro-flat rounded-lg">
            ...
            <p className="text-yellow-600 text-[0.813rem] font-bold">12</p>
            <p className="text-[var(--text-secondary)] mt-1 text-[0.813rem]">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[var(--text-secondary)]">
              Emissão de NF-e para distribuidoras OPME com conformidade
              ANVISA/SEFAZ/LGPD
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="px-4 py-2 rounded-xl neuro-raised flex items-center gap-2">
              ...
              <span className="text-[0.813rem] font-medium">Compliance OK</span>
            </div>

            <div className="px-4 py-2 rounded-xl neuro-raised flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
              <span className="text-[0.813rem] font-medium">SEFAZ Online</span>
            </div>
          </div>
        </div>
