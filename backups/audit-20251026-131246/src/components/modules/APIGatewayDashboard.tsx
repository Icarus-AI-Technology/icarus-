...
export default function APIGatewayDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "endpoints" | "alerts" | "performance"
  >("overview");
  ...
  const handleResetCircuitBreaker = useCallback(
    async (endpointId: string) => {
      if (
        !confirm(
          "Tem certeza que deseja resetar o circuit breaker deste endpoint?",
        )
      )
        return;
  ...
  const avgResponseTime =
    metrics.length > 0
      ? metrics.reduce((sum, m) => sum + m.avg_response_time_ms, 0) /
        metrics.length
      : 0;
  const totalCacheHits = metrics.reduce((sum, m) => sum + m.cache_hits, 0);
  const avgCacheHitRate =
    metrics.length > 0
      ? metrics.reduce((sum, m) => sum + (m.cache_hit_rate_percent || 0), 0) /
        metrics.length
      : 0;
  const successRate =
    totalRequests > 0 ? (totalSuccess / totalRequests) * 100 : 0;

  const endpointsDown = metrics.filter(
    (m) => m.circuit_breaker_state === "open",
  ).length;
  const endpointsHealthy = metrics.filter(
    (m) => m.circuit_breaker_state === "closed",
  ).length;
  const endpointsWarning = metrics.filter(
    (m) => m.circuit_breaker_state === "half_open",
  ).length;

  const criticalAlerts = alerts.filter(
    (a) => a.severidade === "critica" && !a.is_resolved,
  ).length;
  const highAlerts = alerts.filter(
    (a) => a.severidade === "alta" && !a.is_resolved,
  ).length;
  ...
  const renderOverview = () => (
    <div className="space-y-6">
            ...
            {kpis.map((kpi) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <h3 className="opacity-80 text-[0.813rem] font-medium">
                    {kpi.title}
                  </h3>
                  <kpi.icon className="w-5 h-5 opacity-80" />
                </div>
                <div className="mt-4 text-[0.813rem] font-bold">
                  {kpi.value}
                </div>
                {kpi.trend && (
                  <p className="opacity-70 mt-2 text-[0.813rem]">{kpi.trend}</p>
                )}
              </Card>
            ))}
          </div>
      ...
      {/* Alertas Críticos */}
      {alerts.filter(
        (alert) => !alert.is_resolved && alert.severidade === "critica",
      ).length > 0 && (
      ...
      <Card className="p-6 neuro-raised">
        ...
        <OrxBarChart
          data={metrics
            .slice(0, 10)
            .map((m) => ({
              name: m.endpoint_nome,
              sucesso: m.success_count,
              erro: m.error_count,
            }))}
          keys={["sucesso", "erro"]}
          indexBy="name"
                        ...
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <Button
                              ...
                              onClick={() =>
                                handleResetCircuitBreaker(metric.endpoint_id)
                              }
                            />
                          </TooltipTrigger>
                          <TooltipContent>
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <p className="mb-1 text-[0.813rem] font-medium">
                      {alert.mensagem}
                    </p>
        ...
        <Card className="p-6 neuro-raised">
          ...
          <div className="space-y-3">
            <div className="p-4 neuro-inset rounded-lg">
              ...
              <Progress
                value={Math.min((endpointsHealthy / metrics.length) * 100, 100)}
              />
            </div>
            ...
            {endpointsWarning > 0 && (
              <div className="p-4 neuro-inset rounded-lg">
                ...
                <Progress
                  value={Math.min(
                    (endpointsWarning / metrics.length) * 100,
                    100,
                  )}
                />
              </div>
            ...
            {endpointsDown > 0 && (
              <div className="p-4 neuro-inset rounded-lg">
                ...
                <Progress
                  value={Math.min((endpointsDown / metrics.length) * 100, 100)}
                />
              </div>
            )}
          </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[var(--text-secondary)]">
              Gerenciamento de integrações externas: SEFAZ, ANVISA, CFM, Receita
              Federal
            </p>
          </div>
