...
export default function LicitacoesPropostas() {
  ...
  const [activeTab, setActiveTab] = useState<
    "licitacoes" | "propostas" | "dashboard"
  >("dashboard");
  ...
  const carregarTaxaSucesso = async () => {
    try {
      const { data, error } = await supabase.rpc(
        ...
        {
          p_data_inicio: new Date(Date.now() - 365 * 86400000)
            .toISOString()
            .split("T")[0],
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <Card className="p-6 neuro-raised bg-gradient-to-br from-emerald-500 to-emerald-600 text-white">
            ...
            <p className="opacity-80 mt-2 text-[0.813rem]">
              {taxaSucesso?.total_vencidas} de {taxaSucesso?.total_participadas}{" "}
              licitações
            </p>
          </Card>
            ...
            <div className="flex justify-center gap-6 mt-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[0.813rem]">
                  Vencidas ({taxaSucesso?.total_vencidas})
                </span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-error"></div>
                <span className="text-[0.813rem]">
                  Perdidas ({taxaSucesso?.total_perdidas})
                </span>
              </div>
            </div>
          </Card>
              ...
              {licitacoes.slice(0, 5).map((lic) => (
                ...
                >
                  <div className="flex-1">
                    <p className="text-[0.813rem] font-semibold">
                      {lic.titulo}
                    </p>
        ...
        <Table>
          ...
          <TableBody>
            {licitacoes.map((lic) => {
              ...
              const tipoConfig =
                TIPO_LICITACAO[lic.tipo as keyof typeof TIPO_LICITACAO];
      ...
      <div className="space-y-4">
        {propostas.map((prop) => (
          <Card key={prop.id} className="p-6 neuro-raised">
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-[0.813rem] font-medium">
