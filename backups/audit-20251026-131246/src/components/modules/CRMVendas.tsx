...
export default function CRMVendas() {
  ...
  const { leads, loading, error, getTaxaConversao, updateLead, deleteLead } =
    useLeads();
  ...
  const categories: Category[] = useMemo(
    () => [
      ...
      },
      {
        ...
        label: "Oportunidades",
        icon: Target,
        count: leads.filter((l) => l.estagio === stageByCategory.oportunidades)
          .length,
        trend: "+5",
      },
      {
        ...
        label: "Propostas",
        icon: Mail,
        count: leads.filter((l) => l.estagio === stageByCategory.propostas)
          .length,
        trend: "+3",
      },
      {
        ...
        label: "Negociação",
        icon: DollarSign,
        count: leads.filter((l) => l.estagio === stageByCategory.negociacao)
          .length,
        trend: "0",
      },
      {
  ...
  const handleStageChange = useCallback(
    async (id: string, newStage: Lead["estagio"]) => {
      try {
        ...
        addToast(
          `Estágio atualizado para: ${getStageLabel(newStage)}`,
          "success",
        );
      } catch {
                    ...
                    {leadsEstagio.map((lead) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <h4 className="text-[var(--text-primary)] text-body-sm font-medium">
                            {lead.nome}
                          </h4>
                          <span
                ...
                {filteredLeads.map((lead) => (
                  ...
                  >
                    ...
                    <td className="p-4" aria-label="Estágio do lead">
                      <span
                        className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-body-xs font-medium ${getStageColor(lead.estagio)}`}
                      >
                        {getStageLabel(lead.estagio)}
                      </span>
                    </td>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[var(--text-secondary)]">
              Pipeline de vendas e gestão de relacionamento com clientes em
              tempo real
            </p>
          </div>
