...
export function GestaoEntregas({
  ...
  const entregasFiltradas = useMemo(() => {
    ...
    if (filters.transportadora !== "todas") {
      result = result.filter((e) =>
        (e.transportadora ?? "")
          .toLowerCase()
          .includes(filters.transportadora.toLowerCase()),
      );
    }
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`px-2 py-1 rounded-full text-body-xs font-medium ${colors[status] || "bg-[var(--muted)] text-[var(--text-primary)]"}`}
      >
        ...
        <div className="flex flex-wrap items-end gap-4">
          ...
          <div className="w-[180px]">
            ...
            <select
              ...
              onChange={(e) =>
                setFilters({ ...filters, transportadora: e.target.value })
              }
          ...
          <div className="flex gap-2 ml-auto">
            ...
            <button
              onClick={() =>
                setViewMode(viewMode === "lista" ? "mapa" : "lista")
              }
              className="px-4 py-2 rounded-lg
                bg-surface-light dark:bg-surface-dark
                border border-[var(--border)]
