...
export const ObservabilityDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: anomalousUsers } = await supabase.rpc(
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-green-600">
                  {behaviorProfiles.length > 0
                    ? Math.round(
                        (behaviorProfiles.reduce(
                          (sum, p) => sum + (1 - p.taxa_erro_geral),
                          0,
                        ) /
                          behaviorProfiles.length) *
                          100,
                ...
                {systemAlerts.map((alert) => (
                  ...
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium ${getSeverityColor(alert.severidade)}`}
                      >
                        {alert.severidade}
                      </span>
                    </div>
                ...
                {anomalies.map((anomaly, idx) => (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-medium ${getSeverityColor(anomaly.severidade)}`}
                      >
                        {anomaly.severidade}
                      </span>
                    </div>
