...
import {
  NavigationBar,
  NavigationTab,
} from "@/components/oraclusx-ds/NavigationBar";
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <Badge
      variant="default"
      className={cn(variant.bg, variant.color, "font-medium")}
    >
      {variant.label}
    </Badge>
  );
...
export const ConsignacaoAvancada: React.FC = () => {
  ...
  const tabs: NavigationTab[] = [
    ...
    { id: "financeiro", label: "Financeiro", icon: <DollarSign size={18} /> },
    {
      ...
      badge: metricas.hospitaisAtivos,
    },
  ];
  ...
  const hospitaisUnicos = useMemo(
    () => Array.from(new Set(materiais.map((m) => m.hospital_nome))),
    [materiais],
  );
  ...
  const renderHeader = () => (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 className="text-foreground text-body-sm font-extrabold">
          Consignação Avançada
        </h1>
        <p className="text-muted-foreground mt-1">
          Gestão completa de materiais OPME em consignação com controle
        ...
        <NeomorphicCard className="border-l-4 border-red-500">
          ...
          <div className="space-y-3">
            {alertas.slice(0, 3).map((alerta) => (
              <div
                key={alerta.id}
                className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border-l-2 border-red-500"
              >
                <h4 className="text-foreground text-body-sm font-medium">
                  {alerta.titulo}
                </h4>
                ...
                <div className="flex gap-2 mt-2">
                  <Badge
                    variant="default"
                    className="bg-red-100 text-red-800 text-body-sm"
                  >
                    {alerta.severidade}
                  </Badge>
                </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                <th className="text-left p-3 text-foreground text-body-sm font-semibold">
                  Código
                </th>
                <th className="text-left p-3 text-foreground text-body-sm font-semibold">
                  Material
                ...
                <th className="text-right p-3 text-foreground text-body-sm font-semibold">
                  Quantidade
                </th>
                <th className="text-right p-3 text-foreground text-body-sm font-semibold">
                  Valor Total
                </th>
                <th className="text-center p-3 text-foreground text-body-sm font-semibold">
                  Status
                </th>
                <th className="text-center p-3 text-foreground text-body-sm font-semibold">
                  Dias Estoque
            ...
            <tbody>
              {materiais.slice(0, 10).map((material) => (
                ...
                >
                  <td className="p-3 text-foreground font-mono text-body-sm">
                  ...
                  <td className="p-3 text-foreground text-body-sm">
                    {material.hospital_nome}
                  </td>
                  <td className="p-3 text-right text-foreground text-body-sm">
                    {material.quantidade}
                  </td>
                  ...
                  <td className="p-3">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <Button size="sm" variant="default" className="p-2">
                        <Edit size={14} />
                      </Button>
                    </div>
                  </td>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <Badge
                        variant={fat.status === "pago" ? "success" : "warning"}
                      >
                        {fat.status}
                      </Badge>
                    </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <span className="text-foreground font-semibold">
                {metricas.taxaUtilizacao.toFixed(1)}%
              </span>
            </div>
            <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <span className="text-muted-foreground text-body-sm">
                Dias Médio de Estoque
              </span>
              <span className="text-foreground font-semibold">
                {metricas.diasMedioEstoque} dias
              </span>
            </div>
            <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <span className="text-muted-foreground text-body-sm">
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {hospitaisUnicos.map((hospital) => {
          const materiaisHospital = materiais.filter(
            (m) => m.hospital_nome === hospital,
          );
          const valorTotal = materiaisHospital.reduce(
            (sum, m) => sum + m.valor_total,
            0,
          );
            ...
            <NeomorphicCard key={hospital}>
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground text-body-sm">
                    {materiaisHospital.length}{" "}
                    {materiaisHospital.length === 1 ? "material" : "materiais"}
                  </p>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-body-sm">
                  ...
                  <span className="text-foreground">
                    {
                      materiaisHospital.filter((m) => m.status === "disponivel")
                        .length
                    }
                  </span>
                </div>
                <div className="flex justify-between text-body-sm">
                  ...
                  <span className="text-foreground">
                    {
                      materiaisHospital.filter((m) => m.status === "utilizado")
                        .length
                    }
                  </span>
                </div>
  ...
  return (
    <div className="min-h-screen p-6 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      ...
      >
        ...
        <p className="text-muted-foreground text-body-sm">
          Em breve: detalhes completos do material selecionado (ID:{" "}
          {selectedMaterialId ?? "N/A"}).
        </p>
      </Modal>
    </div>
  );
