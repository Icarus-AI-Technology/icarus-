...
export default function BIDashboardInterativo() {
  ...
  const kpis = [
    ...
    },
    {
      ...
      icon: ShoppingCart,
      bgClass:
        "bg-gradient-to-br from-[var(--orx-primary)] to-[var(--orx-primary-hover)]",
      ...
      actions={
        <div className="flex items-center gap-3">
          <Button
            ...
            onClick={() =>
              window.dispatchEvent(new CustomEvent("icarus:refresh-bi"))
            }
          >
          ...
          <Button
            ...
            className="inline-flex items-center gap-2"
            onClick={() =>
              window.dispatchEvent(new CustomEvent("icarus:export-bi"))
            }
          >
        ...
        <div className="neumorphic-card p-6 rounded-2xl bg-[var(--orx-bg-light)] shadow-orx-soft">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-[0.813rem] font-semibold text-[var(--orx-text-primary)]">
                Filtros:
              </span>
            </div>
            <select
        ...
        <div className="grid grid-cols-4 gap-6">
          {kpis.map((kpi, index) => (
            ...
            >
              ...
              <div className="mb-2">
                <p className="text-[0.813rem] font-bold text-[var(--orx-text-primary)] leading-none">
                  {kpi.value}
                </p>
              </div>
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-1">
                  ...
                  <span
                    className={`text-[0.813rem] font-semibold ${kpi.textClass}`}
                  >
                    {kpi.trend}
                  </span>
                </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[rgba(99,102,241,0.1)]">
                  <th className="p-4 text-left text-[0.813rem] font-semibold text-[var(--orx-text-secondary)]">
                    Produto
                  </th>
                  <th className="p-4 text-center text-[0.813rem] font-semibold text-[var(--orx-text-secondary)]">
                    Vendas
                  </th>
                  <th className="p-4 text-right text-[0.813rem] font-semibold text-[var(--orx-text-secondary)]">
                    Receita
                  </th>
                  <th className="p-4 text-center text-[0.813rem] font-semibold text-[var(--orx-text-secondary)]">
                    Margem
              ...
              <tbody>
                {topProdutos.map((produto, index) => (
                  <tr
                    key={index}
                    className="border-b border-[rgba(99,102,241,0.05)]"
                  >
                    <td className="p-4 text-[0.813rem] text-[var(--orx-text-primary)] font-semibold">
                      {produto.nome}
                    </td>
                    <td className="p-4 text-center text-[0.813rem] text-[var(--orx-text-primary)]">
                      {produto.vendas}
                    </td>
                    <td className="p-4 text-right text-[0.813rem] text-[var(--orx-text-primary)] font-semibold">
                      {formatCurrency(produto.receita)}
                    </td>
                    <td className="p-4 text-center text-[0.813rem] text-[var(--orx-success)] font-semibold">
                      {produto.margem}%
                    </td>
