...
import {
  tabelasPrecosService,
  TabelaPreco,
} from "@/lib/services/TabelasPrecosService";
...
export const TabelasPrecos: React.FC = () => {
  ...
  const kpis = {
    ...
    valorTotalMedio:
      tabelas.length > 0
        ? tabelas.reduce((sum, t) => sum + (t.valor_total_estimado || 0), 0) /
          tabelas.length
        : 0,
  };
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      {/* Tabela de Dados */}
      <div className="neumorphic-container rounded-2xl overflow-hidden">
        {loading ? (
                ...
                {tabelasFiltradas.map((tabela) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-right text-[var(--orx-text-primary)] font-semibold text-[0.875rem]">
                      R${" "}
                      {(tabela.valor_total_estimado || 0).toLocaleString(
                        "pt-BR",
                    ...
                    <td className="p-4 text-center text-[var(--orx-text-secondary)] text-[0.813rem]">
                      ...
                      {tabela.data_fim && (
                        <>
                          <br />
                          até{" "}
                          {new Date(tabela.data_fim).toLocaleDateString(
                            "pt-BR",
                          )}
                        </>
                      )}
                    ...
                    <td className="p-4">
                      <div className="flex gap-2 justify-center flex-wrap">
                        <button
                          ...
                          onClick={() =>
                            navigate(`/cadastros/tabelas-precos/${tabela.id}`)
                          }
                          title="Visualizar"
                        >
                          <Eye size={16} />
                        ...
                        <button
                          ...
                          onClick={() =>
                            navigate(
                              `/cadastros/tabelas-precos/${tabela.id}/editar`,
                            )
                          }
                          title="Editar"
                        >
      ...
      <div className="neumorphic-container p-6 rounded-2xl border border-[rgba(59,130,246,0.2)] bg-[rgba(59,130,246,0.05)]">
        <div className="flex gap-3 items-start">
          <AlertTriangle
            size={20}
            className="text-[var(--orx-info)] shrink-0 mt-[2px]"
          />
          <div>
            ...
            <ul className="text-[var(--orx-text-secondary)] text-[0.813rem] leading-6 pl-5 list-disc">
              <li>
                <strong>Fabricante</strong>: Preços de fábrica/custo base
                fornecidos pelos fabricantes
              </li>
              <li>
                <strong>Distribuidor</strong>: Tabela padrão com margem de
                distribuição aplicada
              </li>
              <li>
                <strong>Hospital</strong>: Preços negociados especificamente com
                cada hospital
              </li>
              <li>
                <strong>Convênio</strong>: Tabelas acordadas com planos de saúde
                e seguradoras
              </li>
              <li>
                <strong>Contrato</strong>: Baseadas em contratos formais de
                fornecimento
              </li>
              <li>
                <strong>Licitação</strong>: Preços para participação em
                licitações públicas
              </li>
              <li>
                • Preços podem ter escalonamento por quantidade (descontos
                progressivos)
              </li>
              <li>
                • Todo histórico de alterações é registrado para auditoria
              </li>
            </ul>
          </div>
        </div>
  ...
  return (
    <div className="neumorphic-container p-6 rounded-2xl flex items-center gap-4">
      ...
      <div className="flex-1">
        <p className="text-[var(--orx-text-secondary)] text-[0.813rem] mb-1">
          {label}
        </p>
        <p className="text-[var(--orx-text-primary)] text-2xl font-bold">
          {value}
        </p>
      </div>
    </div>
...
const StatusBadge: React.FC<StatusBadgeProps> = ({ status }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[0.813rem] font-medium bg-[${bg}] text-[${color}]`}
    >
      {icon}
      {label}
    </span>
...
const TipoTabelaBadge: React.FC<TipoTabelaBadgeProps> = ({ tipo }) => {
  const config = {
    fabricante: {
      label: "Fabricante",
      color: "var(--orx-indigo-700)",
      bg: "var(--orx-indigo-50)",
      icon: <Package size={14} />,
    },
    distribuidor: {
      label: "Distribuidor",
      color: "var(--orx-primary)",
      bg: "var(--orx-primary-light)",
      icon: <Users size={14} />,
    },
    hospital: {
      label: "Hospital",
      color: "var(--orx-teal-700)",
      bg: "var(--orx-teal-50)",
      icon: <Building2 size={14} />,
    },
    convenio: {
      label: "Convênio",
      color: "var(--orx-pink-700)",
      bg: "var(--orx-pink-50)",
      icon: <Heart size={14} />,
    },
    contrato: {
      label: "Contrato",
      color: "var(--orx-purple-700)",
      bg: "var(--orx-purple-50)",
      icon: <FileText size={14} />,
    },
    promocional: {
      label: "Promocional",
      color: "var(--orx-orange-700)",
      bg: "var(--orx-orange-50)",
      icon: <Percent size={14} />,
    },
    licitacao: {
      label: "Licitação",
      color: "var(--orx-blue-700)",
      bg: "var(--orx-blue-50)",
