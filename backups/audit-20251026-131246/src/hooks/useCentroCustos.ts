...
export function useCentroCustos() {
  ...
  // Create centro
  const createCentro = useCallback(
    async (
      novoCentro: Omit<
        CentroCusto,
  ...
  const updateCentro = useCallback(
    async (id: string, updates: Partial<CentroCusto>) => {
      try {
        setError(null);
  ...
  const getRealizadoPorCentro = useCallback(
    async (
      ...
      try {
        ...
        const totalRealizado = (
          (despesas as Array<{ valor_original: number }> | null) ?? []
        ).reduce((sum, d) => sum + (d.valor_original ?? 0), 0);

        const orcamento = centroDados.orcamento_mensal;
        const variacao = totalRealizado - orcamento;
        const percentualUtilizado =
          orcamento > 0 ? (totalRealizado / orcamento) * 100 : 0;
        ...
        return {
          ...
          ticket_medio:
            despesas && despesas.length > 0
              ? totalRealizado / despesas.length
              : 0,
          status,
        };
      } catch (error) {
  ...
  const getTodosRealizados = useCallback(
    async (periodo: string): Promise<CentroCustoRealizado[]> => {
      try {
  ...
  const getResumo = useCallback(async (): Promise<CentroCustosResumo> => {
    try {
      ...
      const totalOrcamentoMensal = centrosData.reduce(
        (sum, c) => sum + c.orcamento_mensal,
        0,
      );
      const totalOrcamentoAnual = centrosData.reduce(
        (sum, c) => sum + c.orcamento_anual,
        0,
      );
      ...
      const totalRealizadoMes = realizados.reduce(
        (sum, r) => sum + r.realizado,
        0,
      );
      const variacaoMes = totalRealizadoMes - totalOrcamentoMensal;
      const percentualUtilizado =
        totalOrcamentoMensal > 0
          ? (totalRealizadoMes / totalOrcamentoMensal) * 100
          : 0;

      const centrosAcimaOrcamento = realizados.filter(
        (r) => r.status === "acima_orcamento",
      ).length;
