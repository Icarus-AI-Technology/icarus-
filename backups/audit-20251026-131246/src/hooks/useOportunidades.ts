...
export function useOportunidades() {
  ...
  const updateOportunidade = useCallback(
    async (id: string, updates: Partial<Oportunidade>) => {
      ...
      setState((prev) => ({
        ...prev,
        oportunidades: prev.oportunidades.map((item) =>
          item.id === id ? oportunidade : item,
        ),
      }));
  ...
  const createTarefa = useCallback(
    async (
      payload: Omit<OportunidadeTarefa, "id" | "created_at" | "concluido_em">,
    ) => {
      const { error } = await supabase
        .from("oportunidade_tarefas")
        .insert([payload]);
  ...
  const registrarProposta = useCallback(
    async (
      payload: Omit<OportunidadeProposta, "id" | "criada_em" | "atualizada_em">,
    ) => {
      const { error } = await supabase
        .from("oportunidade_propostas")
        .insert([payload]);
