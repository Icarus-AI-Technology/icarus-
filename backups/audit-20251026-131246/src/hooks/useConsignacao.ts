...
export const useConsignacao = () => {
      ...
      if (filtros.searchTerm && filtros.searchTerm.trim() !== "") {
        ...
        materiaisFiltrados = materiaisFiltrados.filter(
          (m) =>
            ...
            (m.fabricante &&
              m.fabricante.toLowerCase().includes(searchLower)) ||
  ...
  const calcularMetricas = useCallback((): MetricasConsignacao => {
    ...
    const valorTotalConsignado = materiais.reduce(
    ...
    const materiaisUtilizados = materiais.filter(
      (m) => m.status === "utilizado",
    ).length;
    const materiaisDevolvidos = materiais.filter(
      (m) => m.status === "devolvido",
    ).length;
    const materiaisReservados = materiais.filter(
      (m) => m.status === "reservado",
    ).length;

    const taxaUtilizacao =
      totalMateriais > 0 ? (materiaisUtilizados / totalMateriais) * 100 : 0;
    ...
    const diasMedioEstoque =
      totalMateriais > 0
        ? materiais.reduce((sum, m) => sum + (m.dias_estoque || 0), 0) /
          totalMateriais
        : 0;

    const custoCarregamentoTotal = materiais.reduce(
      (sum, m) => sum + (m.custo_carregamento || 0),
      0,
    );
  ...
  const addMaterial = useCallback(
    async (
      material: Omit<
        MaterialConsignado,
        "id" | "created_at" | "updated_at" | "valor_total"
      >,
    ) => {
      setLoading(true);
      try {
  ...
  const updateMaterial = useCallback(
    async (id: string, updates: Partial<MaterialConsignado>) => {
      setLoading(true);
      try {
        const { data, error } = await supabase
          .from("materiais_consignados")
  ...
  const deleteMaterial = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        const err = error as Error;
        const errorMsg =
          err instanceof Error ? err.message : "Erro ao remover material";
  ...
  const registrarMovimentacao = useCallback(
    async (movimentacao: MovimentacaoConsignacao) => {
      try {
        const { data, error } = await supabase
        ...
        if (movimentacao.tipo === "utilizacao") {
          await updateMaterial(movimentacao.material_consignado_id, {
            status: "utilizado",
          });
