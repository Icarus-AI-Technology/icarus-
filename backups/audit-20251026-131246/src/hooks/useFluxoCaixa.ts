...
export function useFluxoCaixa() {
  ...
  const fetchFluxoPeriodo = useCallback(
  ...
  const getResumo = useCallback(
    async (dataInicio: string, dataFim: string): Promise<FluxoCaixaResumo> => {
      try {
        ...
        const totalEntradas = entradasData.reduce(
          (sum, e) => sum + (e.valor_original ?? 0),
          0,
        );
        ...
        const totalSaidas = saidasData.reduce(
          (sum, s) => sum + (s.valor_original ?? 0),
          0,
        );
        ...
        const variacaoPercentual =
          saldoInicial > 0 ? (variacao / saldoInicial) * 100 : 0;
  ...
  const getProjecao = useCallback(
    async (diasFuturos: number): Promise<ProjecaoFluxo[]> => {
      try {
  ...
  // Get cenários
  const getCenarios = useCallback(
    async (diasFuturos: number): Promise<CenarioFluxo[]> => {
      try {
        const { fluxoCaixaAI } = await import("@/lib/services/FluxoCaixaAI");
  ...
  // Get tendência
  const getTendencia = useCallback(async (): Promise<
