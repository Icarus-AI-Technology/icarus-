...
export function useContasReceber() {
  ...
  const updateConta = useCallback(
    async (id: string, updates: Partial<ContaReceber>) => {
      try {
        setError(null);
      ...
      try {
        ...
        const { data, error: updateError } = await supabase
          ...
          .update({
            ...
            status: novoStatus,
            data_pagamento:
              novoStatus === "pago" ? dataPagamento : conta.data_pagamento,
        ...
        const err = error as Error;
        const message =
          err instanceof Error ? err.message : "Erro ao baixar conta";
        setError(message);
  ...
  const getResumo = useCallback(async () => {
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : "Erro ao carregar resumo de contas a receber";
      setError(message);
