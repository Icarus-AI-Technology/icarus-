...
export function useEntregas() {
  ...
  const getEntregaById = useCallback(
    async (id: string): Promise<Entrega | null> => {
      try {
        const { data, error } = await supabase
          .from("entregas")
          .select("*")
  ...
  const getEntregaByCodigo = useCallback(
    async (codigo: string): Promise<Entrega | null> => {
      try {
        const { data, error } = await supabase
          .from("entregas")
          .select("*")
    ...
    async (
      ...
      try {
        ...
        setState((prev) => ({
          ...prev,
          error:
            error instanceof Error ? error.message : "Erro ao criar entrega",
        }));
        return null;
      }
    },
  ...
  const updateEntrega = useCallback(
    async (id: string, updates: Partial<Entrega>): Promise<Entrega | null> => {
      try {
        const { data, error } = await supabase
          .from("entregas")
          .update(updates)
  ...
  const deleteEntrega = useCallback(async (id: string): Promise<boolean> => {
    try {
      ...
      setState((prev) => ({
        ...prev,
        error:
          error instanceof Error ? error.message : "Erro ao deletar entrega",
      }));
      return false;
    }
  }, []);
  ...
  const getHistoricoEntrega = useCallback(
    async (entregaId: string): Promise<EntregaHistorico[]> => {
      try {
        const { data, error } = await supabase
  ...
  const confirmarEntrega = useCallback(
    async (
      id: string,
      assinatura: { nome: string; documento: string },
    ): Promise<boolean> => {
      try {
        const { error } = await supabase
          .from("entregas")
  ...
  const getEstatisticas = useCallback(async () => {
    try {
      ...
      const pendentes = data.filter((e) =>
        ["pendente", "coletado"].includes(e.status),
      ).length;
      const emTransito = data.filter((e) =>
        ["em_transito", "saiu_entrega"].includes(e.status),
      ).length;
      const entregues = data.filter((e) => e.status === "entregue").length;
      const atrasadas = data.filter(
        (e) =>
          ["pendente", "coletado", "em_transito", "saiu_entrega"].includes(
            e.status,
          ) &&
          e.data_previsao &&
