...
export function useFaturas() {
  ...
  const getFaturaById = useCallback(
    async (id: string): Promise<Fatura | null> => {
      try {
        const { data, error } = await supabase
          .from("faturas")
          .select("*")
  ...
  const updateFatura = useCallback(
    async (id: string, updates: Partial<Fatura>): Promise<Fatura | null> => {
      try {
        const { data, error } = await supabase
          .from("faturas")
          .update(updates)
  ...
  const deleteFatura = useCallback(async (id: string): Promise<boolean> => {
    try {
      ...
      setState((prev) => ({
        ...prev,
        error:
          error instanceof Error ? error.message : "Erro ao deletar fatura",
      }));
      return false;
    }
  }, []);
    ...
    async (id: string): Promise<boolean> => {
      try {
        ...
        setState((prev) => ({
          ...prev,
          error:
            error instanceof Error ? error.message : "Erro ao emitir fatura",
        }));
        return false;
      }
    },
  ...
  const cancelarFatura = useCallback(
    async (id: string, motivo: string): Promise<boolean> => {
      try {
        const { error } = await supabase
          .from("faturas")
          .update({
  ...
  const getResumoFaturas = useCallback(async () => {
    try {
      ...
      const emitidas = data.filter((f) =>
        ["emitida", "autorizada"].includes(f.status),
      ).length;
  ...
  const countByStatus = useCallback(async () => {
    try {
      const { data, error } = await supabase.from("faturas").select("status");
