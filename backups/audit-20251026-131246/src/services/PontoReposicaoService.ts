...
export class PontoReposicaoService {
  ...
  static async calcularPontoReposicao(
    ...
    try {
      ...
      return {
        ...
        sugestao_ajuste:
          Math.abs(pontoReposicaoCalculado - (produto.ponto_reposicao || 0)) >
          5,
      };
    } catch (error) {
  ...
  static async verificarPontosReposicao(): Promise<ProdutoAbaixoPonto[]> {
    try {
      ...
      for (const prod of produtos || []) {
        ...
        const consumoMedio = await this.calcularConsumoMedio(
          prod.produto_id,
          30,
        );
    ...
    try {
      ...
      for (const produto of produtosFiltrados) {
        ...
        const { data: produtoInfo } = await supabase
          .from("produtos_opme")
          .select(
            "custo_medio, quantidade_minima_compra, fornecedor_principal_id",
          )
  ...
  static async gerarHistoricoConsumo(
    ...
    try {
      ...
      const variancia =
        valores.reduce((sum, val) => sum + Math.pow(val - mediaDiaria, 2), 0) /
        valores.length;
  ...
  static async recalcularTodosPontos(): Promise<{
    ...
    try {
      ...
      console.log(
        `✅ Recálculo finalizado: ${atualizados}/${produtos.length} atualizados`,
      );
