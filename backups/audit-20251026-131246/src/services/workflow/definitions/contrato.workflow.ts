...
export const CONTRATO_WORKFLOW: WorkflowDefinition = {
  ...
  description:
    "Workflow para gerenciamento de contratos com fornecedores, clientes e parceiros",
  module: "contratos",
  ...
  notifications: [
    ...
    },
    {
      ...
      template:
        "Contrato aguardando aprovação: {tipo} com {parte_nome}. Valor: R$ {valor_total}",
    ...
    },
    {
      ...
      template:
        "Contrato APROVADO! Próximo passo: assinatura. Parte: {parte_nome}",
    ...
    },
    {
      ...
      recipientRoles: ["juridico", "gestor"],
      template:
        "ALERTA: Contrato vence em 30 dias! Iniciar processo de renovação.",
    ...
    },
    {
      ...
      template:
        "Contrato RESCINDIDO! Motivo: {motivo_rescisao}. Parte: {parte_nome}",
      channels: ["email", "whatsapp", "push", "in_app"],
    },
  ],
  ...
  validations: [
    ...
    },
    {
      ...
      type: "approval",
      message:
        "Contratos acima de R$ 100.000 requerem aprovação da presidência",
      validator: async (instance) => {
        const valorTotal = instance.metadata?.valorTotal || 0;
        const aprovadores = instance.metadata?.aprovadores || [];

        if (valorTotal > 100000) {
          return (aprovadores as Array<{ cargo?: string }>).some(
            (a) => a.cargo === "presidente",
          );
        }
        return true;
      },
    },
    {
      ...
      validator: async (instance) => {
        const assinaturas =
          (instance.metadata?.assinaturas as
            | Array<{ assinadoEm?: string }>
            | undefined) || [];
