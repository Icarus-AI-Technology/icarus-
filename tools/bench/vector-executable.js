...
async function runBenchmarks() {
  ...
  try {
    ...
    await benchmarkVectorInsertion(client, 100);
    
    // Teste 2: Busca por similaridade (cosine)
    await benchmarkSimilaritySearch(client, 'cosine', 50);

    // Teste 3: KNN search
    await benchmarkKNNSearch(client, 50);
...
async function setupTestTable(client) {
  console.log('🔧 Configurando tabela de teste...');

  try {
    await client.query('CREATE EXTENSION IF NOT EXISTS vector');
    
    await client.query('DROP TABLE IF EXISTS benchmark_vectors');
...
async function benchmarkVectorInsertion(client, numVectors) {
  ...
  for (let i = 0; i < numVectors; i++) {
    ...
    await client.query(
      'INSERT INTO benchmark_vectors (embedding, category) VALUES ($1, $2)',
      [`[${embedding.join(',')}]`, faker.commerce.department()]
    );
  }
...
async function benchmarkSimilaritySearch(client, metric, numSearches) {
...
async function benchmarkKNNSearch(client, numSearches) {
...
function printResults() {
  ...
  if (avgInsertSpeed > 50 && avgSearchSpeed > 20) {
    console.log('\n✅ Performance EXCELENTE!\n');
  } else if (avgInsertSpeed > 20 && avgSearchSpeed > 10) {
    console.log('\n🟡 Performance BOA\n');
  } else {
