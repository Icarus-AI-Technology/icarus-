...
async function gerarLotes(quantidade: number = 200): Promise<number> {
  ...
  for (let i = 0; i < quantidade; i++) {
    ...
    dataFabricacao.setDate(dataFabricacao.getDate() - Math.floor(Math.random() * 180));
    
    const dataValidade = new Date(dataFabricacao);
    dataValidade.setDate(dataValidade.getDate() + Math.floor(Math.random() * 730) + 365); // 1-3 anos
...
async function gerarCirurgias(quantidade: number = 150): Promise<number> {
  ...
  for (let i = 0; i < quantidade; i++) {
    ...
    dataCirurgia.setDate(dataCirurgia.getDate() - Math.floor(Math.random() * 180) + Math.floor(Math.random() * 60));
 ...
 */
async function gerarDadosFinanceiros(quantidade: number = 200): Promise<number> {
  ...
  for (let i = 0; i < quantidade / 2; i++) {
    ...
    dataVencimento.setDate(dataVencimento.getDate() + Math.floor(Math.random() * 90) - 45);

    const pago = Math.random() > 0.3; // 70% pagos
    const dataPagamento = pago ? new Date(dataVencimento) : null;
    if (dataPagamento) {
      dataPagamento.setDate(dataPagamento.getDate() + Math.floor(Math.random() * 10) - 3);
    }
    ...
    contasReceber.push({
      ...
      status: pago ? 'pago' : (dataVencimento < new Date() ? 'atrasado' : 'aberto'),
 ...
 */
async function gerarMovimentacoesEstoque(quantidade: number = 300): Promise<number> {
  ...
  for (let i = 0; i < quantidade; i++) {
    ...
    dataMovimentacao.setDate(dataMovimentacao.getDate() - Math.floor(Math.random() * 180));
...
export async function gerarDadosRealistasIA(): Promise<DadosGerados> {
  ...
  try {
    ...
    console.log('  💡 Os modelos de IA agora têm dados realistas para treinar.');
    console.log('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');
