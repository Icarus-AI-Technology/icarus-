...
describe('ANVISAService', () => {
  ...
  it('deve lançar erro se registro vazio', async () => {
    await expect(service.buscarProduto('')).rejects.toThrow('Registro obrigatório');
  });
...
describe('CFMService', () => {
  ...
  it('deve lançar erro se CRM vazio', async () => {
    await expect(service.buscarMedico('', 'SP')).rejects.toThrow('CRM e UF obrigatórios');
  });

  it('deve lançar erro se UF vazio', async () => {
    await expect(service.buscarMedico('123456', '')).rejects.toThrow('CRM e UF obrigatórios');
  });
