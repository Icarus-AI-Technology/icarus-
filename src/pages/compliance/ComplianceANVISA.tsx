...
export default function ComplianceANVISA() {
  ...
  const filteredProdutos = produtos.filter(
    (prod) =>
      prod.nome.toLowerCase().includes(searchTerm.toLowerCase()) || prod.lote.includes(searchTerm)
  );
          ...
          {[
            {
              ...
              value: '245',
              icon: Shield,
              color: 'bg-orx-success/10 text-orx-success',
            },
            {
              label: 'Rastreabilidade Parcial',
              value: '12',
              icon: AlertTriangle,
              color: 'bg-orx-warning/10 text-orx-warning',
            },
            {
              label: 'Pendentes',
              value: '3',
              icon: Package,
              color: 'bg-orx-danger/10 text-orx-danger',
            },
          ].map((stat, idx) => (
            <div key={idx} className="bg-orx-bg-surface rounded-xl p-4 shadow-neumo">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="orx-text-2xl orx-orx-font-bold text-orx-text-primary">
                    {stat.value}
                  </p>
                </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span
                      className={`px-3 py-1 orx-text-xs orx-orx-font-medium rounded-lg ${
                        prod.status_rastreabilidade === 'completa'
                          ? 'bg-orx-success/10 text-orx-success'
                          : prod.status_rastreabilidade === 'parcial'
