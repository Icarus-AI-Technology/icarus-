...
>(({ className, value, indicatorColor = 'primary', ...props }, ref) => {
  ...
  return (
    <ProgressPrimitive.Root
      ref={ref}
      className={cn('relative h-4 w-full overflow-hidden rounded-full neuro-inset', className)}
      {...props}
    >
      <ProgressPrimitive.Indicator
        className={cn('h-full w-full flex-1 transition-all', colorClasses[indicatorColor])}
