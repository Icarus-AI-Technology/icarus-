...
const basicItems = [
  ...
  { label: 'Configurações', value: 'settings', icon: <Settings className="w-4 h-4" /> },
  { label: 'Notificações', value: 'notifications', icon: <Bell className="w-4 h-4" /> },
  ...
  args: {
    trigger: (
      <button className="flex items-center gap-2 orx-button px-3 py-2">
        <div className="w-8 h-8 rounded-full bg-[var(--orx-primary)] text-white flex items-center justify-center orx-text-sm orx-orx-font-semibold">
          JD
        </div>
        <span>John Doe</span>
      </button>
    ...
    items: [
      { label: 'Meu Perfil', value: 'profile', icon: <User className="w-4 h-4" /> },
      { label: 'Configurações', value: 'settings', icon: <Settings className="w-4 h-4" /> },
      { type: 'divider' },
      { label: 'Sair', value: 'logout', icon: <LogOut className="w-4 h-4" /> },
    ],
