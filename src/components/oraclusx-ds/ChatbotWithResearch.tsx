...
export const ChatbotWithResearch: React.FC<ChatbotWithResearchProps> = ({
  ...
  const detectUserQuery = (
    query: string
  ): { type: 'user_activities' | 'user_handover'; params: Record<string, string> } | null => {
  ...
  const generateLocalResponse = async (
    query: string,
    context: Array<{ categoria: string; conteudo_texto: string; modulo?: string }>
  ): Promise<string> => {
    if (context.length === 0) {
  ...
  return (
    <div className={cn('fixed z-50', positionClasses[position], className)}>
                ...
                >
                  ...
                  {message.sources && message.sources.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-t-[rgba(0,0,0,0.1)]">
                      <p className="mb-1 text-[0.813rem] text-[var(--orx-text-secondary)] orx-orx-font-medium">
                        Fontes:
                      </p>
      ...
      >
        ...
        <span className="absolute -top-1 -right-1 inline-flex items-center justify-center w-5 h-5 text-[10px] font-bold text-white rounded-full bg-red-500/80 shadow-[0_0_8px_rgba(243,18,96,0.5)]">
          3
        </span>
      </button>
    </div>
