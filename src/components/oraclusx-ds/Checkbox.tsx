    ...
    return (
      <div className={cn('flex items-start gap-3', className)}>
        <div className="relative flex items-center">
          <input ref={ref} id={checkboxId} type="checkbox" className="sr-only peer" {...props} />
          <label
            htmlFor={checkboxId}
            className={cn(
              ...
              'dark:bg-gray-700',
              'peer-checked:bg-primary peer-checked:text-inverse',
              'peer-disabled:opacity-50 peer-disabled:cursor-not-allowed',
              'transition-all duration-150',
              error && 'border-2 border-error'
            )}
          >
            <Check size={14} className="opacity-0 peer-checked:opacity-100 transition-opacity" />
          </label>
        </div>
        {(label || description) && (
