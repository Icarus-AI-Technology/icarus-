/**
 * Módulo 12: Gestão de Leads
 * Pipeline completo de leads e automação comercial
 */

import { useState } from"react";
import { Card } from"@/components/oraclusx-ds";
import { Users, TrendingUp, Target, Phone, Calendar, BarChart3 } from"lucide-react";
import { useDocumentTitle } from"@/hooks";

export default function GestaoLeads() {
  useDocumentTitle("Gestão de Leads");
  const [activeCategory, setActiveCategory] = useState("dashboard");

  const categories = [
    { id:"dashboard", label:"Dashboard", icon: BarChart3, count: 0, trend:"+0" },
    { id:"novos", label:"Novos Leads", icon: Users, count: 0, trend:"+0" },
    { id:"qualificados", label:"Qualificados", icon: Target, count: 0, trend:"+0" },
    { id:"contato", label:"Em Contato", icon: Phone, count: 0, trend:"+0" },
    { id:"convertidos", label:"Convertidos", icon: TrendingUp, count: 0, trend:"+0" },
  ];

  const kpis = [
    { title:"Leads Ativos", value:"0", trend:"+0%", icon: Users, color:"blue" },
    { title:"Taxa Conversão", value:"0%", trend:"+0%", icon: TrendingUp, color:"green" },
    { title:"Lead Score Médio", value:"0", trend:"+0", icon: Target, color:"indigo" },
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-heading-lg font-display text-[var(--text-primary)] mb-2">Gestão de Leads</h1>
            <p className="text-[var(--text-secondary)]">
              Pipeline completo de leads com scoring automático e integração CRM
            </p>
          </div>
          <div className="px-4 py-2 rounded-xl neuro-raised">
            <span className="text-body-sm text-[var(--text-primary)]" style={{ fontWeight: 500 }}>0 Leads</span>
          </div>
        </div>
        ...
        <Card className="neuro-raised p-12 text-center">
          ...
          <h3 className="text-heading-sm text-[var(--text-primary)] mb-2" style={{ fontWeight: 500 }}>Gestão de Leads</h3>
