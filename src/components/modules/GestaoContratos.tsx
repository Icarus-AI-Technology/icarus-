...
const statusBadgeMap: Record<string, { label: string; variant:"success" |"warning" |"info" |"error" |"neutral" }> = {
...
export default function GestaoContratos() {
  ...
  const [contratoAlertas, setContratoAlertas] = useState<Awaited<ReturnType<typeof getAlertasContrato>>>([]);
  ...
  useEffect(() => {
    ...
    const loadDetails = async () => {
      ...
      try {
        const [clausulasData, aditivosData, slasData, aprovacoesData, alertasData] = await Promise.all([
  ...
  }, [selectedContratoId, getClausulas, getAditivos, getSLAs, getAprovacoes, getAlertasContrato, addToast]);
  ...
  const handleRegistrarAprovacao = async (contratoId: string, nivel: Parameters<typeof registrarAprovacao>[1], status:"aprovado" |"rejeitado") => {
    try {
    ...
    <div className="grid gap-6">
        ...
        {dashboardData.map((card) => (
          <Card key={card.id} className="neuro-raised h-full">
            <CardHeader className="flex-row items-start justify-between gap-4">
              <div>
                <CardTitle className="text-[var(--text-primary)] text-body-sm" style={{ fontWeight: 500 }}>
                  {card.label}
                </CardTitle>
            ...
            <CardContent>
              ...
              <Badge variant={card.intent} className="mt-4 w-fit">
                Atualizado {kpis?.atualizado_em ? formatDistanceToNow(new Date(kpis.atualizado_em), { addSuffix: true, locale: ptBR }) :"agora"}
              </Badge>
            </CardContent>
          </Card>
        ))}
      ...
      <Card className="neuro-raised">
        ...
        <CardContent className="space-y-3">
          {alertas.length === 0 && (
            <p className="text-body-sm text-[var(--text-secondary)]">Nenhum alerta crítico no momento.</p>
          )}
          {alertas.map((alerta) => (
            <Alert
              key={alerta.id}
              ...
              title={alerta.numero_contrato ? `${alerta.numero_contrato} - ${alerta.contrato_titulo}` : alerta.contrato_titulo ??"Contrato"}
            />
          ))}
        </CardContent>
      </Card>
  ...
  const renderContratosAtivos = () => (
    <Table
      columns={[
        {
          ...
          cell: ({ row }) => (
            <div className="space-y-1">
              <p className="text-body-sm text-[var(--text-primary)]" style={{ fontWeight: 500 }}>{row.titulo}</p>
        ...
        },
        {
          ...
          accessor:"status",
          cell: ({ row }) => {
            const badge = statusBadgeMap[row.status] ?? statusBadgeMap.neutral;
        ...
        },
        {
          header:"Tipo",
          accessor:"tipo",
          cell: ({ row }) => <Badge variant="default">{row.tipo.replaceAll("_","")}</Badge>,
        },
        {
          header:"Ações",
          accessor:"id",
          cell: ({ row }) => (
            <div className="flex gap-2">
              ...
              <Dropdown
                trigger={<Button variant="default" size="sm">•••</Button>}
                items={[
                  {
                    id:"alerta",
      ...
      data={contratosOrdenados.filter((contrato) => contrato.status ==="ativo" || contrato.status ==="renovacao")}
      emptyState={{
        icon: FileText,
  ...
  const renderContratosVencendo = () => (
    <div className="space-y-4">
      {contratosVencendo.length === 0 && <p className="text-body-sm text-[var(--text-secondary)]">Sem contratos próximos do vencimento.</p>}
      {contratosVencendo.map((contrato) => (
        <Card key={contrato.id} className="neuro-raised">
          <CardContent className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-4">
            <div>
              <p className="text-body-sm text-[var(--text-primary)]" style={{ fontWeight: 500 }}>{contrato.titulo}</p>
            ...
            <div className="flex items-center gap-2">
              <Badge variant="warning">Vence em {formatDistanceToNow(new Date(contrato.data_fim), { addSuffix: true, locale: ptBR })}</Badge>
              <Button variant="default" size="sm" onClick={() => handleRegistrarAlerta(contrato.id,"Contrato a vencer","vencimento","critical")}>
                Criar alerta
              </Button>
            </div>
          </CardContent>
      ...
      {selectedContratoId ? (
                  ...
                  <div key={aprovacao.id} className="flex flex-col md:flex-row md:items-center justify-between gap-2 p-3 rounded-lg neuro-inset">
                    <div>
                      ...
                      <p className="text-body-xs text-[var(--text-secondary)]">
                        {aprovacao.status ==="aprovado" ?"Aprovado" : aprovacao.status ==="rejeitado" ?"Rejeitado" :"Pendente"}
                        {aprovacao.aprovado_em && ` • ${formatDistanceToNow(new Date(aprovacao.aprovado_em), { addSuffix: true, locale: ptBR })}`}
                      </p>
                      {aprovacao.comentario && (
                        <p className="text-body-xs text-[var(--text-secondary)] mt-1">{aprovacao.comentario}</p>
                      )}
                    </div>
                    {aprovacao.status ==="pendente" && (
                      <div className="flex gap-2">
                        <Button size="sm" variant="default" onClick={() => handleRegistrarAprovacao(selectedContratoId, aprovacao.nivel,"aprovado")}>
                          Aprovar
                        </Button>
                        <Button size="sm" variant="default" onClick={() => handleRegistrarAprovacao(selectedContratoId, aprovacao.nivel,"rejeitado")}>
                          Rejeitar
                        </Button>
                      </div>
                    )}
            ...
            <Card className="neuro-raised">
              ...
              <CardContent className="space-y-3">
                {contratoAlertas.map((item) => (
                  <Alert
                    key={item.id}
                    type={item.severidade ==="critical" ?"error" : item.severidade ==="warning" ?"warning" :"info"}
                    title={`${item.tipo.toUpperCase()} • ${formatterDate(item.data_alerta)}`}
                    message={item.descricao ??"Sem descrição"}
                  />
                ))}
                {contratoAlertas.length === 0 && (
                  <p className="text-body-sm text-[var(--text-secondary)]">Nenhum alerta registrado.</p>
                )}
              </CardContent>
            </Card>
          </div>
        ...
        <Card className="neuro-raised">
          <CardContent className="py-12 text-center">
            <p className="text-body-sm text-[var(--text-secondary)] mb-4">Selecione um contrato na lista para visualizar o workflow de aprovação.</p>
            ...
            <div className="space-y-4">
              {clausulas.map((clausula) => (
                <div key={clausula.id} className="p-4 rounded-lg neuro-inset">
                  <p className="text-body-sm text-[var(--text-primary)]" style={{ fontWeight: 500 }}>{clausula.titulo}</p>
                  <p className="text-body-xs text-[var(--text-secondary)] mt-2 whitespace-pre-line">{clausula.texto}</p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-body-sm text-[var(--text-secondary)]">Selecione um contrato para visualizar as cláusulas.</p>
          )}
        </CardContent>
      </Card>
          ...
          {selectedContratoId && aditivos.length > 0 ? (
            <Table
              columns={[
                ...
                { header:"Assinado em", accessor:"data_assinatura", cell: ({ row }) => formatterDate(row.data_assinatura) },
                { header:"Valor ajuste", accessor:"valor_ajuste", cell: ({ row }) => formatterCurrency.format(row.valor_ajuste ?? 0) },
              ]}
              data={aditivos}
            />
          ) : (
    ...
    <Card className="neuro-raised">
      ...
      <CardContent>
        {selectedContratoId && slas.length > 0 ? (
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">Selecione um contrato para visualizar os SLAs.</p>
        )}
      </CardContent>
    </Card>
  );
      ...
      <CardContent>
        <Table
          columns={[
            ...
            { header:"Valor", accessor:"valor_total", cell: ({ row }) => formatterCurrency.format(row.valor_total ?? 0) },
            { header:"Renovação automática", accessor:"renovacao_automatica", cell: ({ row }) => (row.renovacao_automatica ?"Sim" :"Não") },
            { header:"Índice", accessor:"indice_reajuste", cell: ({ row }) => prazoLabels[row.indice_reajuste ??"nenhum"] },
            { header:"Prazo aviso", accessor:"prazo_aviso_rescisao", cell: ({ row }) => (row.prazo_aviso_rescisao ? `${row.prazo_aviso_rescisao} dias` :"—") },
          ]}
  ...
  const renderEncerrados = () => (
    <Table
      columns={[
        ...
        { header:"Encerrado em", accessor:"updated_at", cell: ({ row }) => formatterDate(row.updated_at) },
        { header:"Valor total", accessor:"valor_total", cell: ({ row }) => formatterCurrency.format(row.valor_total ?? 0) },
      ]}
      data={contratosOrdenados.filter((contrato) => contrato.status ==="encerrado" || contrato.status ==="cancelado")}
      emptyState={{
        icon: AlertTriangle,
        title:"Nenhum contrato encerrado",
        description:"Histórico ficará disponível aqui quando houver contratos concluídos ou cancelados.",
      }}
    />
  );
  ...
  const renderIA = () => (
    <Card className="neuro-raised">
      <CardHeader className="flex-row items-center justify-between">
        ...
        <Button variant="default" size="sm" disabled={!selectedContratoId || iaLoading} onClick={() => selectedContratoId && handleExecutarAnaliseIA(selectedContratoId)}>
      ...
      <CardContent className="min-h-[200px]">
        {iaInsights ? (
          ...
          <p className="text-body-sm text-[var(--text-secondary)]">Execute a análise para gerar recomendações.</p>
        )}
      </CardContent>
    </Card>
  );
  ...
  const renderRelatorios = () => (
    <Card className="neuro-raised">
      ...
      <CardContent className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {[
          { id:"vencimento", label:"Contratos vencendo", description:"CSV com contratos que vencem nos próximos 90 dias" },
          { id:"slas", label:"Indicadores SLA", description:"Resumo dos SLAs cumpridos e descumpridos" },
          { id:"financeiro", label:"Financeiro consolidado", description:"Visa a integração com Financeiro Avançado" },
        ].map((item) => (
          <Card key={item.id} className="neuro-inset">
            <CardContent className="pt-6">
              <p className="text-body-sm text-[var(--text-primary)]" style={{ fontWeight: 500 }}>{item.label}</p>
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        <header className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div>
            <h1 className="text-heading-lg font-display text-[var(--text-primary)]">Gestão de Contratos</h1>
            <p className="text-body-sm text-[var(--text-secondary)] mt-2">
              Controle completo de contratos de fornecimento, OPME, SLA e renovações com IA e alertas proativos.
            </p>
          </div>
          <div className="flex gap-2">
            <Button variant="default" size="sm">
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={index} className="flex items-center justify-center h-40 rounded-2xl border border-dashed border-[var(--border-muted)]">
                <Spinner size="md" />
              </div>
            ))}
          </div>
