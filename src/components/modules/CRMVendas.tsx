...
export function CRMVendas() {
  ...
  const filteredLeads = useMemo(() => leads.filter((lead) => {
    ...
    const matchTab =
      ...
      activeTab ==="ativos" ? !["ganho","perdido","desqualificado"].includes(lead.estagio) :
      activeTab ==="ganhos" ? lead.estagio ==="ganho" :
      activeTab ==="perdidos" ? lead.estagio ==="perdido" :
      false;
  ...
  const renderPipeline = () => {
    const estagios: Lead["estagio"][] = ["novo","contato","qualificado","proposta","negociacao","ganho","perdido","desqualificado",
    ];
              ...
              <div key={estagio} className="flex-1 min-w-[280px]">
                <Card className="neuro-raised p-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-[var(--text-primary)] orx-orx-font-medium">{getStageLabel(estagio)}</h3>
                    ...
                    {leadsEstagio.map((lead) => (
                      ...
                      >
                        <div className="flex items-start justify-between mb-2">
                          <h4 className="text-[var(--text-primary)] text-body-sm orx-orx-font-medium">{lead.nome}</h4>
                          <span className={`px-2 py-0.5 rounded-full text-body-xs ${getStageColor(lead.estagio)}`}>
                            {getStageLabel(lead.estagio)}
                          </span>
                        </div>

                        <p className="text-body-sm text-[var(--text-secondary)] mb-3">{lead.empresa ||"Sem empresa"}</p>
            ...
            <table className="w-full">
              <thead className="border-b border-[var(--border)]">
                <tr>
                  ...
                  <th className="text-left p-4 text-body-sm text-[var(--text-secondary)] orx-orx-font-medium">Empresa</th>
                  <th className="text-left p-4 text-body-sm text-[var(--text-secondary)] orx-orx-font-medium">Contato</th>
                  <th className="text-left p-4 text-body-sm text-[var(--text-secondary)] orx-orx-font-medium">Valor Est.</th>
                  <th className="text-left p-4 text-body-sm text-[var(--text-secondary)] orx-orx-font-medium">Prob.</th>
                  <th className="text-left p-4 text-body-sm text-[var(--text-secondary)] orx-orx-font-medium">Estágio</th>
                  ...
                  >
                    <td className="p-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[var(--text-primary)] orx-orx-font-medium">{lead.nome}</span>
                      </div>
                    </td>
                    ...
                    <td className="p-4" aria-label="Estágio do lead">
                      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-body-xs orx-orx-font-medium ${getStageColor(lead.estagio)}`}>
                        {getStageLabel(lead.estagio)}
                      </span>
                    </td>
                    <td className="p-4">
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-heading-lg font-display text-[var(--text-primary)] mb-2">CRM & Vendas</h1>
        ...
        {activeCategory ==="relatorios" && (
          <Card className="neuro-raised p-12 text-center">
            ...
            <h3 className="text-heading-sm text-[var(--text-primary)] mb-2 orx-orx-font-medium">Relatórios de Vendas</h3>
