...
export default function FaturamentoNFeCompleto() {
  ...
  const [activeTab, setActiveTab] = useState<
    'dashboard' | 'emitir' | 'consultar' | 'rastreabilidade'
  >('dashboard');
  ...
  const loadKPIs = useCallback(async () => {
    try {
      ...
      const valor_total_mes = data?.reduce((sum: number, nfe: { valor_total: number }) => sum + nfe.valor_total, 0) || 0;
      const autorizadas = data?.filter((nfe: { status: string }) => nfe.status === 'autorizada').length || 0;
      const rejeitadas = data?.filter((nfe: { status: string }) => nfe.status === 'rejeitada').length || 0;
      const taxa_autorizacao = total_mes > 0 ? (autorizadas / total_mes) * 100 : 0;

      // Conformidade ANVISA (média de % de produtos rastreados)
      const conformidade_anvisa =
        data?.reduce(
          (sum: number, nfe: { rastreabilidade_anvisa?: { percentual_conformidade?: number } }) => sum + (nfe.rastreabilidade_anvisa?.percentual_conformidade || 0),
          0
        ) / (total_mes || 1);
  ...
  const _emitirNFe = async (nfeData: Partial<NFe>) => {
    setLoading(true);
    try {
      // 1. Validações
      ...
      const produtosSemRastreabilidade = nfeData.produtos.filter(
        (p) => !p.anvisa_registro || !p.lote || !p.data_validade
      );
      ...
      const valor_produtos = nfeData.produtos.reduce((sum: number, p: ProdutoNFe) => sum + p.valor_total, 0);
      const valor_total_tributos = nfeData.produtos.reduce(
        (sum: number, p: ProdutoNFe) => sum + p.icms_valor + (p.ipi_valor || 0) + p.pis_valor + p.cofins_valor,
        0
      );
      ...
      const nfeCompleta: NFe = {
        ...
        valor_total:
          valor_produtos +
          (nfeData.valor_frete || 0) +
          (nfeData.valor_seguro || 0) -
          (nfeData.valor_desconto || 0) +
          (nfeData.valor_outras_despesas || 0),
        rastreabilidade_anvisa: {
          produtos_rastreados: nfeData.produtos.filter(
            (p) => p.anvisa_registro && p.lote && p.data_validade
          ).length,
      ...
      nfeCompleta.rastreabilidade_anvisa.percentual_conformidade =
        (nfeCompleta.rastreabilidade_anvisa.produtos_rastreados /
          nfeCompleta.rastreabilidade_anvisa.total_produtos) *
        100;
      ...
      const { data: resultadoSEFAZ, error: erroSEFAZ } = await supabase.functions.invoke(
        'emitir-nfe-sefaz',
        {
      ...
      addToast(`Erro ao emitir NF-e: ${err.message}`, 'error');
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const cancelarNFe = async (nfeId: string, motivo: string) => {
    ...
    try {
      ...
      const { data: resultadoCancelamento, error } = await supabase.functions.invoke(
        'cancelar-nfe-sefaz',
        {
      ...
      await supabase.from('contas_receber').update({ status: 'cancelado' }).eq('nfe_id', nfeId);
      ...
      addToast(`Erro ao cancelar NF-e: ${err.message}`, 'error');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1 text-yellow-700 dark:text-yellow-300 text-[0.813rem]">
                ...
                {kpis.rejeitadas > 5 && (
                  <li>
                    • {kpis.rejeitadas} NF-es rejeitadas este mês: Revise cadastros e dados fiscais
                  </li>
                )}
              </ul>
            </div>
      ...
      <div className="space-y-4">
        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <p className="text-blue-700 dark:text-blue-300 text-[0.813rem]">
            Todos os dispositivos médicos (OPME) comercializados devem ter rastreabilidade completa:
            número de registro ANVISA, lote, data de fabricação e validade.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-4 neuro-flat rounded-lg">
            ...
            <p className="text-yellow-600 text-[0.813rem] orx-orx-font-bold">12</p>
            <p className="text-[var(--text-secondary)] mt-1 text-[0.813rem]">
          ...
          <div className="p-4 neuro-flat rounded-lg">
            ...
            <p className="text-[var(--text-secondary)] mt-1 text-[0.813rem]">Meta: ≥ 95%</p>
          </div>
        </div>
      </div>
    </Card>
        ...
        <div className="flex items-start justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="px-4 py-2 rounded-xl neuro-raised flex items-center gap-2">
              ...
              <span className="text-[0.813rem] orx-orx-font-medium">Compliance OK</span>
            </div>

            <div className="px-4 py-2 rounded-xl neuro-raised flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
              <span className="text-[0.813rem] orx-orx-font-medium">SEFAZ Online</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-2 p-2 neuro-inset rounded-2xl w-fit">
          {[
            ...
            <button
              key={tab.id}
              onClick={() =>
                setActiveTab(tab.id as 'dashboard' | 'emitir' | 'consultar' | 'rastreabilidade')
              }
              className={`
