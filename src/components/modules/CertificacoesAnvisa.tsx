...
import { Shield, FileText, CheckCircle, Award, Settings, TrendingUp } from"lucide-react";
export const CertificacoesAnvisa: React.FC = () => {
  ...
  const categories = [
    ...
    { id:"config", label:"Config", icon: Award, count: 34, trend:"+5" }
  ];
  const kpis = [
    { title:"Total", value:"303", trend:"+40", icon: Shield, color:"blue" },
    { title:"Taxa", value:"99.5%", trend:"+2%", icon: FileText, color:"green" },
    { title:"IA", value:"99.5%", trend:"on", icon: Settings, color:"indigo" },
    ...
    <div className="min-h-screen p-6 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      <div className="max-w-7xl mx-auto">
        <header className="mb-6 flex items-start justify-between">
          <div><h1 className="text-heading-lg font-display text-primary dark:text-gray-100 mb-2">Certificações ANVISA IA</h1><p className="text-secondary dark:text-muted">Sistema IA</p></div>
          <div className="flex items-center gap-2 px-4 py-2 bg-primary rounded-full"><Settings className="text-inverse animate-spin-slow" size={20} /><div className="text-left"><p className="text-inverse text-body-sm" style={{ fontWeight: 500 }}>IA</p><p className="text-indigo-100 text-body-xs">99.5%</p></div></div>
        </header>
        ...
        <Card><CardHeader><CardTitle>Certificações ANVISA IA</CardTitle></CardHeader><CardContent><div className="text-center py-12"><Shield size={64} className="mx-auto text-muted mb-4" /><p className="text-secondary dark:text-muted">Módulo IA</p></div></CardContent></Card>
      </div>
    </div>
  );
};
export default CertificacoesAnvisa;
