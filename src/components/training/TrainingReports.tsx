    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="orx-text-3xl orx-font-bold">{overallStats.totalLessons}</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="orx-text-3xl orx-font-bold">{overallStats.completed}</p>
        ...
        <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="orx-text-3xl orx-font-bold">{Math.round(overallStats.avgScore)}</p>
        </div>
        ...
        <div className="bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg shadow-lg p-6 text-white">
          ...
          <p className="orx-text-3xl orx-font-bold">{formatTime(overallStats.totalTime)}</p>
        </div>
      </div>

      {/* Progress by Module */}
      <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
        <h2 className="orx-text-xl orx-font-bold text-gray-900 mb-6 flex items-center gap-2">
          ...
          {progressByModule.map((progress) => (
            <div key={progress.modulo} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="orx-font-semibold text-gray-900">{progress.modulo}</h3>
              ...
              <div className="grid grid-cols-3 gap-4 orx-text-sm">
                <div>
                  ...
                  <p className="orx-font-semibold text-blue-600">{Math.round(progress.progresso)}%</p>
                </div>
                <div>
                  <p className="text-gray-600">Pontuação Média</p>
                  <p className="orx-font-semibold text-purple-600">{Math.round(progress.pontuacao_media)}</p>
                </div>
                <div>
                  <p className="text-gray-600">Tempo Gasto</p>
                  <p className="orx-font-semibold text-orange-600">{formatTime(progress.tempo_total)}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          {recentLessons.map((lesson) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="orx-font-medium text-gray-900">{lesson.licao}</h4>
              ...
              <div className="text-right">
                {lesson.concluido ? (
                  <div>
                    <p className="orx-text-sm orx-font-semibold text-green-600">Concluído</p>
                    <p className="orx-text-xs text-gray-500">Pontuação: {lesson.pontuacao}</p>
                  </div>
                ) : (
                  <div>
                    <p className="orx-text-sm orx-font-semibold text-gray-600">Em progresso</p>
