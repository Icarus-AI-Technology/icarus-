...
function AppShell() {
  ...
  useEffect(() => {
    ...
    const raf = requestAnimationFrame(() => {
      ...
      if (link) {
        link.addEventListener('mouseenter', handler, { once: true } as AddEventListenerOptions);
        link.addEventListener('focus', handler, { once: true } as AddEventListenerOptions);
      }
    });
    return () => {
  ...
  useEffect(() => {
    const loaders: Record<string, () => Promise<unknown>> = {
      ...
      '/configuracoes': () => import('./components/modules/ConfiguracoesSistema'),
    };
            ...
            >
              ...
              <Suspense fallback={<SkeletonRouteFallback /> }>
                <Routes>
                  ...
                  <Route path="/qa/cirurgias" element={<CirurgiasProcedimentos />} />
