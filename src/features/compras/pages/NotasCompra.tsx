...
export const NotasCompra: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[0.813rem] font-semibold ${bgClass}`}>
        {config.icon}
        {config.label}
      </span>
    );
    ...
    >
          ...
          return (
            <div key={index} className="p-6 rounded-2xl bg-[var(--orx-bg-light)] shadow border border-white/10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-[0.813rem] font-bold text-[var(--orx-text-primary)] mb-1">{kpi.value}</div>
        ...
        <div className="grid [grid-template-columns:repeat(auto-fit,minmax(300px,1fr))] gap-4">
          ...
          <label className="p-8 rounded-2xl bg-[var(--orx-bg-light)] shadow border-2 border-dashed border-[var(--orx-primary)] cursor-pointer flex flex-col items-center gap-4 transition-all hover:-translate-y-0.5">
            ...
            <div className="text-center">
              <h3 className="text-[0.813rem] font-semibold text-[var(--orx-text-primary)] mb-2">Importar XML NF-e</h3>
          ...
          <label className="p-8 rounded-2xl bg-[var(--orx-bg-light)] shadow border-2 border-dashed border-[var(--orx-info-dark)] cursor-pointer flex flex-col items-center gap-4 transition-all hover:-translate-y-0.5">
            ...
            <div className="text-center">
              <h3 className="text-[0.813rem] font-semibold text-[var(--orx-text-primary)] mb-2">OCR de DANFE</h3>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className={`p-6 rounded-2xl border ${validacaoStatus.sefaz ? 'bg-[var(--orx-success-light)] border-[var(--orx-success-dark)]' : 'bg-[var(--orx-error-light)] border-[var(--orx-error-dark)]'}`}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className={`text-[0.813rem] font-semibold ${validacaoStatus.sefaz ? 'text-[var(--orx-success-dark)]' : 'text-[var(--orx-error-dark)]'}`}>
            ...
            <ul className="list-none p-0 flex flex-col gap-2">
              <li className="flex items-center gap-2">
                {validacaoStatus.sefaz ? (
                  <CheckCircle size={16} className="text-[var(--orx-success-dark)]" />
                ) : (
          ...
          <div className="p-6 rounded-2xl bg-[var(--orx-bg-light)] shadow border border-white/10">
            <h3 className="text-[0.813rem] font-semibold text-[var(--orx-text-primary)] mb-4">Dados da Nota Fiscal</h3>
      ...
      <div className="rounded-2xl bg-[var(--orx-bg-light)] shadow border border-white/10 overflow-hidden">
        <div className="p-6 border-b border-black/5">
          <h3 className="text-[0.813rem] font-semibold text-[var(--orx-text-primary)]">Notas Fiscais Recentes</h3>
        </div>
            ...
            <thead>
              <tr className="bg-indigo-500/5">
                {['Nº NF-e','Fornecedor','Data Emissão','Valor Total','Status','Ações'].map((h, idx) => (
                  <th key={idx} className={`p-4 ${idx === 5 ? 'text-center' : 'text-left'} text-[0.813rem] font-semibold text-[var(--orx-text-secondary)] border-b border-black/5`}>
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {notas.map((nota) => (
                <tr key={nota.id} className="border-b border-black/5 hover:bg-indigo-500/5 transition-colors">
                  <td className="p-4 text-[0.813rem] font-semibold text-[var(--orx-primary)]">{nota.numero_nfe}</td>
                  ...
                  <td className="p-4 text-[0.813rem] font-semibold text-[var(--orx-text-primary)]">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(nota.valor_total)}</td>
