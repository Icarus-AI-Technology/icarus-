...
export const PedidosCompra: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-[0.813rem] font-semibold ${bgClass[config.bg]} ${textClass[config.text]}`}>
        {config.icon}
        {config.label}
      </span>
    );
          ...
          return (
            <div key={index} className="p-6 rounded-2xl bg-[var(--orx-bg-light)] shadow-[var(--orx-shadow-light-1),_var(--orx-shadow-light-2)] border border-white/10 transition">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`flex items-center gap-1 text-[0.813rem] font-semibold ${trendClass}`}>
                ...
                pedidosFiltrados.map((pedido) => (
                  <tr key={pedido.id} className="border-b border-black/5 hover:bg-[rgba(99,102,241,0.03)] transition-colors">
                    ...
                    <td className="p-4 text-[0.813rem] text-[var(--orx-text-primary)]">
                      <div>
                        <div className="font-semibold">{pedido.fornecedor_nome}</div>
