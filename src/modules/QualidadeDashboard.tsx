...
import { Shield, AlertTriangle, TrendingUp, FileText, Bell } from 'lucide-react';
...
export default function QualidadeDashboard() {
  ...
  const scoreGeral = checklists.length > 0
    ...
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: 'var(--orx-text-secondary)' }}>
          6 Agentes de IA + 4 Modelos Preditivos + Integração APIs Regulatórias (ANVISA, NOTIVISA, SIGTAP)
        </p>
      </div>
      ...
      <div style={{
        ...
        <div className="neumorphic-card" style={{ padding: '1.5rem', textAlign: 'center' }}>
          ...
          />
          <div style={{ marginTop: '1rem', fontSize: '0.875rem', fontWeight: 600, color: 'var(--orx-text-primary)' }}>
            Score Geral BPD
          </div>
        ...
        <div className="neumorphic-card" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
            ...
            <div>
              <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--orx-text-primary)' }}>6</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)' }}>Agentes Ativos</div>
            </div>
          </div>
          <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)', lineHeight: '1.6' }}>
            • Padronização<br/>
            • Rastreabilidade<br/>
            • Qualificação<br/>
            • Tecnovigilância<br/>
        ...
        <div className="neumorphic-card" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
            ...
            <div>
              <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--orx-text-primary)' }}>4</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)' }}>Modelos IA</div>
            </div>
          </div>
          <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)', lineHeight: '1.6' }}>
            • Risco de Glosa<br/>
            • Falhas Fornecedor<br/>
            • Estoque Crítico<br/>
            • Score Conformidade
          </div>
        </div>
        ...
        <div className="neumorphic-card" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem' }}>
            ...
            <div>
              <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--orx-text-primary)' }}>3</div>
              <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)' }}>APIs Governamentais</div>
            </div>
          </div>
          <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)', lineHeight: '1.6' }}>
            • ANVISA DataVisa<br/>
          ...
          {checklists.map((checklist, idx) => (
            <div key={idx} className="neumorphic-card" style={{ padding: '1.5rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--orx-text-primary)', marginBottom: '0.25rem' }}>
                    {checklist.categoria}
                  </h3>
                </div>
                <div style={{
                ...
                {checklist.itens.map((item, i) => (
                  <div key={i} style={{
                    ...
                    border: `1px solid ${
                      ...
                      item.status === 'nao_conforme' ? 'var(--orx-error)' : 'var(--orx-warning)'
                    }`
                  }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: '0.875rem', color: 'var(--orx-text-primary)', marginBottom: '0.25rem' }}>
                        {item.descricao}
                      </div>
                      {item.evidencia && (
                        <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)' }}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid var(--orx-border)' }}>
                <th style={{ padding: '0.75rem', textAlign: 'left', color: 'var(--orx-text-primary)', fontSize: '0.875rem', fontWeight: 600 }}>
                  Agente
                </th>
                <th style={{ padding: '0.75rem', textAlign: 'left', color: 'var(--orx-text-primary)', fontSize: '0.875rem', fontWeight: 600 }}>
                  Função
                </th>
                <th style={{ padding: '0.75rem', textAlign: 'center', color: 'var(--orx-text-primary)', fontSize: '0.875rem', fontWeight: 600 }}>
                  Status
                </th>
              </tr>
            </thead>
            <tbody>
              {[
                { nome: 'Agente de Padronização', funcao: 'Automatiza processo de padronização de materiais e atualização de catálogos com registro ANVISA' },
                { nome: 'Agente de Rastreabilidade', funcao: 'Verifica se lote tem número ANVISA, validade, lote e paciente vinculados. Gera alertas automáticos.' },
                { nome: 'Agente de Qualificação', funcao: 'Avalia cumprimento dos requisitos de qualidade dos fornecedores. Gera score de 0-100.' },
                { nome: 'Agente de Tecnovigilância', funcao: 'Monitora eventos técnicos e adversos. Integração com NOTIVISA para notificação automática.' },
                { nome: 'Agente de Justificativas OPME', funcao: 'Gera textos automáticos de justificativa de uso conforme RN 465/2021 da ANS.' },
                { nome: 'Agente de Auditoria Interna', funcao: 'Automatiza checklists de auditoria, sugere ações preventivas e corretivas.' },
              ].map((agente, i) => (
                <tr key={i} style={{ borderBottom: '1px solid var(--orx-border)' }}>
                  ...
                  <td style={{ padding: '1rem', fontSize: '0.8125rem', color: 'var(--orx-text-secondary)', lineHeight: '1.5' }}>
                    {agente.funcao}
                  </td>
            ...
            <div key={i} className="neumorphic-card" style={{ padding: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '1rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: '0.8125rem', color: 'var(--orx-text-secondary)', lineHeight: '1.5', marginBottom: '0.75rem' }}>
                    {modelo.descricao}
                  </p>
                  <div style={{
                    padding: '0.5rem',
      ...
      <div style={{ marginTop: '2rem' }}>
        ...
        <div style={{
          ...
          {[
            { nome: 'ANVISA DataVisa', status: '✅ Integrado', descricao: 'Consultas de produtos e empresas' },
            { nome: 'NOTIVISA', status: '✅ Integrado', descricao: 'Notificação de eventos adversos' },
            { nome: 'SIGTAP', status: '✅ Integrado', descricao: 'Tabela de procedimentos SUS' },
          ].map((api, i) => (
            <div key={i} className="neumorphic-card" style={{ padding: '1rem' }}>
              <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--orx-text-primary)', marginBottom: '0.5rem' }}>
                {api.nome}
              </div>
              <div style={{ fontSize: '0.75rem', color: 'var(--orx-text-secondary)', marginBottom: '0.5rem' }}>
                {api.descricao}
              </div>
