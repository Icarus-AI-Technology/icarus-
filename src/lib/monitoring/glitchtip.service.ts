...
export class GlitchTipService {
  ...
  constructor(config?: {
    ...
    this.environment = config?.environment || process.env.VITE_ENVIRONMENT || 'production';
    this.release = config?.release || process.env.VITE_RELEASE || undefined;
    this.enabled = config?.enabled ?? (!!this.dsn && this.environment !== 'development');
   ...
   */
  private async sendTransaction(transaction: PerformanceTransaction): Promise<void> {
