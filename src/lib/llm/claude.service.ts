...
export class ClaudeService {
  ...
  async analyzeDocument(
    ...
    const systemPrompts = {
      compliance: 'Você é um auditor especializado em compliance regulatório ANVISA e OPME.',
      risk: 'Você é um especialista em análise de riscos hospitalares e gestão OPME.',
      financial: 'Você é um analista financeiro especializado em gestão hospitalar.',
      clinical: 'Você é um profissional da saúde especializado em procedimentos OPME.',
    };
...
Forneça sua análise no seguinte formato JSON:
{
  ...
  "confidence": 0.0-1.0
    ...
    const { content } = await this.generate(prompt, systemPrompts[analysisType], {
      temperature: 0.3,
      max_tokens: 2000,
    });
