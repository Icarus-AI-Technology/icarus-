...
export const ConsignacaoAvancada: React.FC = () => {
  ...
  const renderHeader = () => (
    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6">
      <div>
        <h1 className="text-foreground text-body-sm orx-orx-font-extrabold">Consignação Avançada</h1>
      ...
      {alertas.length > 0 && (
        <NeomorphicCard className="border-l-4 border-red-500">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-foreground orx-orx-font-semibold">Alertas de Conferência Semanal</h3>
          ...
          <div className="space-y-3">
            {alertas.slice(0, 3).map(alerta => (
              <div key={alerta.id} className="p-3 bg-red-50 dark:bg-red-900/20 rounded-lg border-l-2 border-red-500">
                <h4 className="text-foreground text-body-sm orx-orx-font-medium">{alerta.titulo}</h4>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-700">
                <th className="text-left p-3 text-foreground text-body-sm orx-orx-font-semibold">Código</th>
                <th className="text-left p-3 text-foreground text-body-sm orx-orx-font-semibold">Material</th>
                <th className="text-left p-3 text-foreground text-body-sm orx-orx-font-semibold">Hospital</th>
                <th className="text-right p-3 text-foreground text-body-sm orx-orx-font-semibold">Quantidade</th>
                <th className="text-right p-3 text-foreground text-body-sm orx-orx-font-semibold">Valor Total</th>
                <th className="text-center p-3 text-foreground text-body-sm orx-orx-font-semibold">Status</th>
                <th className="text-center p-3 text-foreground text-body-sm orx-orx-font-semibold">Dias Estoque</th>
                <th className="text-right p-3 text-foreground text-body-sm orx-orx-font-semibold">Ações</th>
              </tr>
            </thead>
            <tbody>
              {materiais.slice(0, 10).map((material) => (
                <tr key={material.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                  ...
                  <td className="p-3">
                    <div>
                      <div className="text-foreground text-body-sm orx-orx-font-medium">{material.nome}</div>
                      {material.lote && (
                  ...
                  <td className="p-3 text-right text-foreground text-body-sm orx-orx-font-medium">
              ...
              {faturamentos.map(fat => (
                <div key={fat.id} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="text-foreground orx-orx-font-medium">{fat.numero_fatura}</h4>
                      <p className="text-muted-foreground text-body-sm">{fat.hospital_nome}</p>
                    </div>
                    <div className="text-right">
                      <div className="text-foreground orx-orx-font-semibold">{formatCurrency(fat.valor_liquido)}</div>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              ...
              <span className="text-foreground orx-orx-font-semibold">{metricas.taxaUtilizacao.toFixed(1)}%</span>
            </div>
            <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <span className="text-muted-foreground text-body-sm">Dias Médio de Estoque</span>
              <span className="text-foreground orx-orx-font-semibold">{metricas.diasMedioEstoque} dias</span>
            </div>
            <div className="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
              <span className="text-muted-foreground text-body-sm">Hospitais Ativos</span>
              <span className="text-foreground orx-orx-font-semibold">{metricas.hospitaisAtivos}</span>
            </div>
          </div>
        </CardContent>
      </NeomorphicCard>
            ...
            <NeomorphicCard key={hospital}>
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  <h3 className="text-foreground orx-orx-font-semibold">{hospital}</h3>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-body-sm">
                  ...
                  <span className="text-foreground orx-orx-font-semibold">{formatCurrency(valorTotal)}</span>
                </div>
                <div className="flex justify-between text-body-sm">
