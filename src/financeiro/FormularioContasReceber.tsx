...
const schemaContaReceber = z.object({
  ...
  forma_recebimento: z.enum(['dinheiro', 'pix', 'boleto', 'transferencia', 'cartao_credito', 'cartao_debito', 'cheque']).optional(),
  numero_parcela: z.number().optional(),
  total_parcelas: z.number().optional(),
  status: z.enum(['pendente', 'recebido', 'atrasado', 'cancelado', 'parcial', 'protesto']).optional(),
...
export default function FormularioContasReceber() {
  ...
  const { register, control, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormContaReceberData>({
    resolver: zodResolver(schemaContaReceber),
    defaultValues: { tipo: 'venda', status: 'pendente', forma_recebimento: 'boleto', valor_juros: 0, valor_desconto: 0 }
  });
  ...
  const secoes = [
      ...
      campos: (
        <div className={FORM_GRID}>
          ...
          <FormField id="descricao" label="Descrição" required error={errors.descricao?.message} className={FORM_COL.full}>
            <NeuInput id="descricao" placeholder="Ex: Venda de materiais OPME - Cirurgia 001" error={!!errors.descricao} {...register('descricao')} />
          </FormField>
        </div>
      )
    },
    {
      ...
      campos: (
        <div className={FORM_GRID}>
          <FormField id="cliente_nome" label="Nome do Cliente" required error={errors.cliente_nome?.message} className={FORM_COL.twoThirds}>
            <NeuInput id="cliente_nome" placeholder="Ex: Hospital São Lucas" error={!!errors.cliente_nome} {...register('cliente_nome')} />
          </FormField>

          <FormField id="cliente_cnpj" label="CNPJ/CPF">
            <NeuInput id="cliente_cnpj" placeholder="00.000.000/0000-00" {...register('cliente_cnpj')} />
          </FormField>
        </div>
      )
    },
      ...
      campos: (
        <div className={FORM_GRID}>
          ...
          <FormField id="valor_recebido" label="Valor Recebido (R$)">
            <NeuInput id="valor_recebido" type="number" step="0.01" placeholder="0.00" {...register('valor_recebido', { valueAsNumber: true })} />
          </FormField>
        </div>
      )
    },
      ...
      campos: (
        <div className={FORM_GRID}>
          <FormField id="forma_recebimento" label="Forma de Recebimento">
            <Controller name="forma_recebimento" control={control} render={({ field }) => (
              <NeuSelect value={field.value} onValueChange={field.onChange} options={[
