...
export function useConvenios() {
  ...
  // Create convenio
  const createConvenio = useCallback(async (novoConvenio: Omit<Convenio,"id" |"created_at" |"updated_at" |"empresa_id">) => {
    try {
      setError(null);
  ...
  const getEstatisticasConvenio = useCallback(async (convenioId: string) => {
    try {
      ...
      const lotesData = (lotes as Array<{ valor_total: number; valor_glosado: number }> | null) ?? [];
  ...
  const getResumo = useCallback(async (): Promise<ConveniosResumo> => {
    try {
      ...
      const comFaturamentoEletronico = conveniosData.filter((c) => c.faturamento_eletronico);

      const valorFaturado = conveniosData.reduce((sum, c) => sum + (c.total_faturado || 0), 0);
      const valorGlosas = conveniosData.reduce((sum, c) => sum + (c.total_glosas || 0), 0);

      const conveniosComTaxa = conveniosData.filter((c) => c.taxa_glosa !== undefined && c.taxa_glosa > 0);
      const taxaGlosaMedia = conveniosComTaxa.length > 0
        ? conveniosComTaxa.reduce((sum, c) => sum + (c.taxa_glosa || 0), 0) / conveniosComTaxa.length
        : 0;
