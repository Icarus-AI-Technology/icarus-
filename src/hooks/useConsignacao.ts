...
export const useConsignacao = () => {
  ...
  const calcularMetricas = useCallback((): MetricasConsignacao => {
    ...
    const taxaUtilizacao = totalMateriais > 0 ? (materiaisUtilizados / totalMateriais) * 100 : 0;
    ...
    return {
      ...
      valorTotalEstoque: valorDisponivel,
  ...
  const addMaterial = useCallback(
    async (
      material: Omit<MaterialConsignado, 'id' | 'created_at' | 'updated_at' | 'valor_total'>
    ) => {
      setLoading(true);
      try {
        const { data, error } = await supabase
  ...
  const deleteMaterial = useCallback(
    async (id: string) => {
      setLoading(true);
      try {
        const { error } = await supabase.from('materiais_consignados').delete().eq('id', id);
