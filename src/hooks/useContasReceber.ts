...
export function useContasReceber() {
  ...
  const createConta = useCallback(
    async (novaConta: Omit<ContaReceber, 'id' | 'created_at' | 'updated_at' | 'empresa_id'>) => {
      try {
        setError(null);
  ...
  const deleteConta = useCallback(async (id: string) => {
    try {
      setError(null);
      const { error: deleteError } = await supabase.from('contas_receber').delete().eq('id', id);
  ...
  const getResumo = useCallback(async () => {
    try {
      setError(null);
      const { data, error: resumoError } = await supabase.rpc('contas_receber_resumo');
      ...
      const err = error as Error;
      const message =
        err instanceof Error ? err.message : 'Erro ao carregar resumo de contas a receber';
      setError(message);
  ...
  const getContasPorRisco = useCallback(
    async (risco: ContaReceber['risco_inadimplencia']) => {
