...
export interface MovimentacaoEstoque {
  ...
  tipo: 'entrada' | 'saida' | 'transferencia' | 'ajuste' | 'devolucao' | 'perda';
  quantidade: number;
  lote?: string;
  serie?: string;
  
  motivo: 'compra' | 'venda' | 'cirurgia' | 'transferencia' | 'ajuste_inventario' | 
...
export const useEstoque = () => {
  ...
  const registrarMovimentacao = useCallback(async (movimentacao: MovimentacaoEstoque) => {
    setLoading(true);
    setError(null);
      ...
      if (movimentacao.tipo === 'entrada') {
        ...
        if (estoqueExistente) {
          ...
          await supabase
            .from('estoque')
            .update({
              quantidade: estoqueExistente.quantidade + movimentacao.quantidade
            })
        ...
        if (estoqueExistente) {
          await supabase
            .from('estoque')
            .update({
              quantidade: Math.max(0, estoqueExistente.quantidade - movimentacao.quantidade)
            })
            .eq('id', estoqueExistente.id);
        }
      }
  ...
  const criarReserva = useCallback(async (reserva: ReservaEstoque) => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      if (!estoqueAtual || estoqueAtual.quantidade_disponivel < reserva.quantidade) {
