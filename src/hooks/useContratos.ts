...
export interface Contrato {
  ...
  indice_reajuste:"nenhum" |"ipca" |"igpm" |"inpc" |"percentual_fixo" | null;
...
export function useContratos() {
  ...
  const createContrato = useCallback(
    async (payload: Omit<Contrato,"id" |"created_at" |"updated_at"> & { clausulas?: ContratoClausula[]; slas?: ContratoSLA[] }) => {
      ...
      if (slas?.length) {
        await supabase.from("contratos_sla").insert(
  ...
  const registrarAlerta = useCallback(
    async (alerta: Omit<ContratoAlerta,"id" |"created_at">) => {
      const { error } = await supabase.from("contratos_alertas").insert([alerta]);
      if (error) throw error;
      await fetchAlertas();
    },
    [fetchAlertas]
