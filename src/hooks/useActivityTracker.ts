...
export const useActivityTracker = () => {
  ...
  const trackActivity = useCallback(
    async (activity: ActivityData) => {
      ...
      try {
        ...
        const rota =
          activity.rota ?? (typeof window !== 'undefined' ? window.location.pathname : undefined);
  ...
  const trackPageView = useCallback(
    (modulo: string, sub_modulo?: string) => {
      trackActivity({
  ...
  const trackSearch = useCallback(
    (modulo: string, query: string, resultados: number) => {
      trackActivity({
        acao: 'buscar',
        modulo,
        metodo: 'SEARCH',
  ...
  const trackExport = useCallback(
    (modulo: string, formato: string, total_registros: number) => {
      trackActivity({
        acao: 'exportar',
        modulo,
        metodo: 'EXPORT',
  ...
  const trackAIInteraction = useCallback(
    (modulo: string, pergunta: string, resposta: string, feedback?: number) => {
      trackActivity({
        acao: 'interacao_ia',
        modulo,
