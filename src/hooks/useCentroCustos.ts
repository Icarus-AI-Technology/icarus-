...
export function useCentroCustos() {
  ...
  const fetchCentros = useCallback(async (filters?: CentroCustosFilters) => {
    try {
      ...
      let query = supabase.from('centro_custos').select('*').order('codigo', { ascending: true });
  ...
  const createCentro = useCallback(
    async (novoCentro: Omit<CentroCusto, 'id' | 'created_at' | 'updated_at' | 'empresa_id'>) => {
      try {
        setError(null);
  ...
  const deleteCentro = useCallback(async (id: string) => {
    try {
      setError(null);
      const { error: deleteError } = await supabase.from('centro_custos').delete().eq('id', id);
  ...
  const getResumo = useCallback(async (): Promise<CentroCustosResumo> => {
    try {
      ...
      const percentualUtilizado =
        totalOrcamentoMensal > 0 ? (totalRealizadoMes / totalOrcamentoMensal) * 100 : 0;
