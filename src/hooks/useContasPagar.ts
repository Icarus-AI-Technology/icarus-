...
export function useContasPagar() {
  ...
  const createConta = useCallback(
    async (novaConta: Omit<ContaPagar, 'id' | 'created_at' | 'updated_at' | 'empresa_id'>) => {
      try {
        setError(null);
  ...
  const deleteConta = useCallback(async (id: string) => {
    try {
      setError(null);
      const { error: deleteError } = await supabase.from('contas_pagar').delete().eq('id', id);
  ...
  const agendarPagamento = useCallback(
    async (id: string, dataAgendamento: string, formaPagamento: string, bancoId: string) => {
      try {
        setError(null);
        const { data, error: updateError } = await supabase
          .from('contas_pagar')
  ...
  const getResumo = useCallback(async () => {
    try {
      setError(null);
      const { data, error: resumoError } = await supabase.rpc('contas_pagar_resumo');
      ...
      const err = error as Error;
      const message =
        err instanceof Error ? err.message : 'Erro ao carregar resumo de contas a pagar';
      setError(message);
