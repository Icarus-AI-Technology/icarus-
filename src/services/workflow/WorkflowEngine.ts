...
export class WorkflowEngine {
  ...
  registerWorkflow(definition: WorkflowDefinition): void {
    ...
    console.log(`✅ Workflow registrado: ${definition.name} (${definition.id})`);
  }
  ...
  async createInstance(
    ...
    await this.triggerNotifications(workflow, instance, 'state_enter', initialState.id);
   ...
   */
  canTransition(instance: WorkflowInstance, toStateId: string): {
    allowed: boolean;
    reason?: string;
  } {
  ...
  async transition(
    ...
    const fromState = workflow.states.find(s => s.id === instance.currentStateId);
    ...
    // Executar validações
    const validations = workflow.validations?.filter(v => v.stateId === toStateId);
    if (validations) {
    ...
    // Notificações
    await this.triggerNotifications(workflow, updatedInstance, 'state_exit', fromState.id);
    await this.triggerNotifications(workflow, updatedInstance, 'state_enter', toStateId);
  ...
  getNextStates(instance: WorkflowInstance): WorkflowState[] {
    ...
    return workflow.states.filter(s => currentState.allowedTransitions.includes(s.id));
  }
  ...
  private async triggerNotifications(
    ...
    const rules = workflow.notifications.filter(
      rule => rule.trigger === trigger && (!stateId || rule.stateId === stateId)
    );
   ...
   */
  async calculateMetrics(workflowId: string, instances: WorkflowInstance[]): Promise<WorkflowMetrics> {
    const workflow = this.getWorkflowDefinition(workflowId);
    if (!workflow) {
      throw new Error('Workflow não encontrado');
    }
    
    const filteredInstances = instances.filter(i => i.workflowId === workflowId);
    ...
    const averageCompletionTime = completedInstances.length > 0
      ? completedInstances.reduce((sum, i) => sum + this.getTotalTime(i), 0) / completedInstances.length / (1000 * 60 * 60) // em horas
      : 0;
    
    // Distribuição por estado
    const stateDistribution: Record<string, number> = {};
    for (const instance of filteredInstances) {
      stateDistribution[instance.currentStateId] = (stateDistribution[instance.currentStateId] || 0) + 1;
    }
    
    // Identificar gargalos
    const bottleneckStates = workflow.states.map(state => {
      const instancesInState = filteredInstances.filter(i => i.currentStateId === state.id);
      const avgTime = instancesInState.length > 0
        ? instancesInState.reduce((sum, i) => sum + this.getTimeInCurrentState(i), 0) / instancesInState.length / (1000 * 60 * 60)
        : 0;
    ...
    const cancelledInstances = filteredInstances.filter(i => i.currentStateId.includes('cancel'));
    ...
    return {
      ...
      stateDistribution,
      completionRate: (completedInstances.length / filteredInstances.length) * 100,
      cancellationRate: (cancelledInstances.length / filteredInstances.length) * 100,
    };
  }
}
