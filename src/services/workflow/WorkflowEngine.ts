...
export class WorkflowEngine {
   ...
   */
  async setDueDate(instance: WorkflowInstance, dueDate: Date): Promise<WorkflowInstance> {
    return {
      ...instance,
      dueDate,
      updatedAt: new Date(),
  ...
  private async triggerNotifications(
    ...
    for (const rule of rules) {
      ...
      console.log(`🔔 Notificação: ${trigger} - ${rule.template} → ${recipients.join(', ')}`);
   ...
   */
  private resolveRecipients(instance: WorkflowInstance, rule: NotificationRule): string[] {
      ...
      .map((state) => {
        ...
        const avgTime =
          instancesInState.length > 0
            ? instancesInState.reduce((sum, i) => sum + this.getTimeInCurrentState(i), 0) /
