...
export class CorreiosService {
  ...
  async rastrear(codigoRastreio: string): Promise<RastreioResponse> {
    try {
      ...
      console.error('Erro ao rastrear Correios:', err);
      throw new Error(`Falha ao rastrear encomenda: ${err.message}`);
    }
  }
  ...
  async calcularFrete(params: FreteParams): Promise<FreteResponse[]> {
    try {
      ...
      throw new Error(`Falha ao calcular frete: ${err.message}`);
    }
  }
  ...
  async consultarCEP(cep: string): Promise<{
    ...
    try {
      ...
      throw new Error(`CEP não encontrado: ${err.message}`);
    }
  }
  ...
  async criarPostagem(params: {
    ...
    try {
      ...
      throw new Error(`Falha ao criar postagem: ${err.message}`);
    }
  }
