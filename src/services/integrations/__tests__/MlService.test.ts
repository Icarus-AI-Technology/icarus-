...
describe('MlService', () => {
  ...
  it('calls optimizer endpoint', async () => {
    mock.onPost(`${baseUrl}/optimizer/or-tools`).reply(200, { optimum: [1, 2, 3] });
  ...
  it('performs vector search', async () => {
    mock.onPost(`${baseUrl}/vector/faiss/search`).reply(200, { ids: ['a'], scores: [0.95] });
  ...
  it('clears vectors', async () => {
    mock.onPost(`${baseUrl}/vector/faiss/clear`).reply(200, { status: 'cleared' });
  ...
  it('forecasts series', async () => {
    mock
      ...
      .reply(200, { forecast: [1, 2], timestamps: ['2025-10-20', '2025-10-21'] });
  ...
  it('lists persisted vectors', async () => {
    ...
    const result = await MlService.listPersistedVectors({ module: 'cirurgias' }, endpoint);
