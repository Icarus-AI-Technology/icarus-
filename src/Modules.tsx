...
export default function ModulesPage() {
  ...
  const getStatusBadge = (status: Module['status']) => {
    ...
    return (
      <span className={`px-2 py-1 rounded-full text-body-xs orx-orx-font-medium ${badges[status]}`}>
        {labels[status]}
      </span>
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="neumorphic-card">
        ...
        <p className="text-muted-foreground">
          Sistema enterprise com {modules.length} módulos especializados em gestão OPME
        </p>
      </div>
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className="p-3 rounded-lg bg-primary/10">
                        <Icon size={24} className="text-primary dark:text-indigo-400" />
                      </div>
