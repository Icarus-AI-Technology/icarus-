...
async function main() {
  ...
  try {
    ...
    log('='.repeat(80), 'cyan');
    log(`Migrations aplicadas:     ${applied}/${MIGRATIONS_FASE4.length}`, applied === MIGRATIONS_FASE4.length ? 'green' : 'yellow');
    ...
    log(`\n📈 Completude do schema:  ${completude}% (${afterCount}/104)`, completude >= 80 ? 'green' : 'yellow');
