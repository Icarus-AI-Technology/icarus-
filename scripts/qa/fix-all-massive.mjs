...
function limparClasses(content) {
  const classesToRemove = [
    ...
    'font-thin', 'font-light', 'font-medium', 'font-semibold', 'font-bold', 'font-extrabold', 'font-black'
  ];
  ...
  classesToRemove.forEach(cls => {
    ...
    result = result.replace(new RegExp(`className="([^"]*)\\s+${cls}\\s+([^"]*)"`, 'g'), 'className="$1 $2"');
  ...
  for (const file of files) {
    try {
      ...
      if (modified !== original) {
        ...
        const changes = (original.match(/text-(xs|sm|lg|xl|2xl|3xl|4xl|5xl)|font-(thin|light|medium|semibold|bold|extrabold|black)/g) || []).length;
